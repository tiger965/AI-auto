...
def run_data_preprocessing_example():
    ...
    try:
        ...
        normalized_sequence = processor.normalize_sequence(
            sequence_data,
            method='z-score'  # 使用Z-score标准化
        )
        
        print(f"序列数据预处理后: {normalized_sequence}")
        print(f"均值: {np.mean(normalized_sequence):.4f}, 标准差: {np.std(normalized_sequence):.4f}")
...
def run_configuration_example():
    ...
    print(f"批处理大小从 {original_batch_size} 修改为 {custom_config.training.batch_size}")
