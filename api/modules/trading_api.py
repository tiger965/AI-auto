...
class TradingAPI(BaseAPI):
    ...
    def register_endpoints(self, bp: Blueprint) -> None:
        ...
        def build_strategy():
            ...
            try:
                # 验证输入数据
                if 'entry_logic' not in data or 'exit_logic' not in data:
                    return jsonify({"error": "缺少必填字段 (entry_logic, exit_logic)"}), 400
                if 'risk' not in data or not isinstance(data['risk'], dict):
                    return jsonify({"error": "缺少风险参数 (risk)"}), 400
                if 'stop_loss_pct' not in data['risk'] or 'take_profit_pct' not in data['risk']:
                    return jsonify({"error": "风险参数不完整 (stop_loss_pct, take_profit_pct)"}), 400
                
                # 生成策略ID
                strategy_id = self._generate_strategy_id(data['symbol'], data['timeframe'])
                ...
                return jsonify({
                    ...
                    "functions": ["populate_entry_trend", "populate_exit_trend"]
                }), 201
