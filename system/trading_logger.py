...
class TradingLogger:
    ...
    def __init__(
        ...
        self.rotator = LogRotator(
            log_dir, max_size_mb=rotation_size_mb, max_files=max_files
        )
    ...
    def log_strategy(self, strategy_id, event, data=None, level=LogLevel.INFO):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to write log for strategy {strategy_id}: {str(e)}"
            )
            return False
    ...
    def log_execution(self, strategy_id, action, details, level=LogLevel.INFO):
        ...
        # 记录日志
        return self.log_strategy(strategy_id, f"EXECUTION_{action}", details, level)
    ...
    def log_error(self, strategy_id, error_msg, details=None):
        ...
        self.tracker.update_status(
            strategy_id, "ERROR", {"error_msg": error_msg, "details": details}
        )
        ...
        return self.log_strategy(
            strategy_id,
            "ERROR",
            {"message": error_msg, "details": details},
            LogLevel.ERROR,
        )
...
if __name__ == "__main__":
    ...
    logger.log_strategy(
        strategy_id,
        "SIGNAL",
        {"symbol": "AAPL", "direction": "BUY", "strength": 0.75, "price": 150.25},
    )
    ...
    logger.log_error(
        ...
        {"api": "trading_broker", "timeout": 30, "retry_count": 3},
    )
