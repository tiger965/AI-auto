...
class MemoryCache:
    ...
    def _cleanup_expired(self):
        ...
        with self.lock:
            # Find expired keys
            expired_keys = [
                key for key, item in self.items.items() if item.is_expired()
            ]
            ...
            if expired_keys:
                logger.debug(
                    f"Removed {len(expired_keys)} expired items from {self.name} cache"
                )
    ...
    def set(self, key: str, value: Any, ttl: Optional[float] = None) -> None:
        ...
        with self.lock:
            ...
            if self.max_size is not None and item.size > self.max_size:
                logger.warning(
                    f"Item size ({item.size} bytes) exceeds cache max size ({self.max_size} bytes)"
                )
                return
        ...
        with self.lock:
            ...
            return {
                ...
                "usage_percent": (
                    ...
                    100 if self.max_size else None
                ),
                "hits": self.hits,
                "misses": self.misses,
                "hit_rate": hit_rate,
...
class DiskCache:
    ...
    def __init__(
        self, name: str, cache_dir: str = None, max_size: Optional[int] = None
    ):
        """
        ...
        self.cache_dir = cache_dir or os.path.join(
            os.path.expanduser("~"), ".cache", "ai_system", name
        )
        self.max_size = max_size
        self.current_size = 0
        self.index = {}
        ...
        if os.path.exists(index_path):
            try:
                ...
                self.current_size = sum(
                    item.get("size", 0) for item in self.index.values()
                )

                logger.debug(
                    f"Loaded disk cache index: {len(self.index)} items, {self.current_size} bytes"
                )
            except Exception as e:
    ...
    def _cleanup_expired(self):
        ...
        with self.lock:
            ...
            if expired_keys:
                ...
                self._save_index()
                logger.debug(
                    f"Removed {len(expired_keys)} expired items from {self.name} disk cache"
                )
    ...
    def _make_space(self, needed_space: int):
        ...
        # Sort items by last accessed time
        sorted_items = sorted(
            self.index.items(), key=lambda x: x[1].get("last_accessed", 0)
        )
    ...
    def set(self, key: str, value: Any, ttl: Optional[float] = None) -> None:
        ...
        with self.lock:
            ...
            if self.max_size is not None and item_size > self.max_size:
                logger.warning(
                    f"Item size ({item_size} bytes) exceeds cache max size ({self.max_size} bytes)"
                )
                return
...
def set(
    key: str, value: Any, ttl: Optional[float] = None, cache_name: Optional[str] = None
) -> None:
    """
    Store an item in the cache.
