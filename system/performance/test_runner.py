...
from system.performance import test_cases
...
def run_predefined_suite(
    suite_name: str, save_results: bool = True, output_file: str = None
):
    """Run a predefined test suite."""
    print(f"Running predefined test suite: {suite_name}")
    try:
        results = tc.run_test_suite(
            suite_name=suite_name, save_results=save_results, output_file=output_file
        )
...
def create_and_run_custom_suite(save_results: bool = True, output_file: str = None):
    ...
    while add_more.lower() == "y":
        ...
        try:
            ...
            duration = input(
                "Test duration in seconds (leave empty for default): ")
            if duration:
        ...
        if test_type == "cpu":
            num_processes = input(
                "Number of CPU processes (leave empty for default): ")
            if num_processes:
                try:
            ...
            try:
                allocation_size = input(
                    "Allocation size in MB (leave empty for default): "
                )
                if allocation_size:
                    config["allocation_size_mb"] = int(allocation_size)

                allocation_count = input(
                    "Number of allocations (leave empty for default): "
                )
                if allocation_count:
            ...
            try:
                file_size = input(
                    "File size in MB (leave empty for default): ")
                if file_size:
                    config["file_size_mb"] = int(file_size)

                block_size = input(
                    "Block size in KB (leave empty for default): ")
                if block_size:
            ...
            protocol = input(
                "Protocol (tcp/udp, leave empty for default): ").lower()
            if protocol in ["tcp", "udp"]:
                config["protocol"] = protocol

            try:
                packet_size = input(
                    "Packet size in KB (leave empty for default): ")
                if packet_size:
            ...
            try:
                cache_size = input(
                    "Cache size in MB (leave empty for default): ")
                if cache_size:
                    config["cache_size_mb"] = int(cache_size)

                cache_ops = input(
                    "Number of cache operations (leave empty for default): "
                )
                if cache_ops:
    ...
    results = tc.run_test_suite(
        custom_suite=custom_suite, save_results=save_results, output_file=output_file
    )
    print_summary(results)
...
def main(argv: List[str]):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hs:co:n", ["help", "suite=",
                              "custom", "output=", "no-save", "list"]
        )
...
def example_usage():
    ...
    baseline_results = tc.run_test_suite(
        "comprehensive", output_file="baseline_results.json"
    )
