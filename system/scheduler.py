    ...
    def execute(self) -> Any:
        ...
        try:
            if self.timeout is not None:
                ...
                try:
                    result_type, result = result_queue.get(timeout=self.timeout)
                    if result_type == "error":
                        raise result
                    return result
                except queue.Empty:
                    thread.join(0.1)  # Give the thread a little time to clean up
                    self.status = "timeout"
                    raise TimeoutError(f"Task {self.name} timed out after {self.timeout} seconds")
            else:
                # Execute directly
            ...
            if self.retry_count <= self.max_retries:
                logger.warning(f"Task {self.name} failed, will retry ({self.retry_count}/{self.max_retries}): {str(e)}")
...
class CronTask(ScheduledTask):
    ...
    def __init__(self, 
        ...
        self.minute, self.hour, self.day, self.month, self.day_of_week = self._parse_cron(cron_expr)
    ...
    def _parse_cron(self, cron_expr: str) -> Tuple[Set[int], Set[int], Set[int], Set[int], Set[int]]:
        """
        ...
        if len(fields) != 5:
            raise ValueError(f"Invalid cron expression: {cron_expr}. Expected 5 fields, got {len(fields)}.")
        ...
        for part in field.split(','):
            if '-' in part:
                ...
                if start < min_val or end > max_val:
                    raise ValueError(f"Invalid range {start}-{end} for field with range {min_val}-{max_val}")
                ...
                if value < min_val or value > max_val:
                    raise ValueError(f"Invalid value {value} for field with range {min_val}-{max_val}")
                result.add(value)
    ...
    def _calculate_next_run(self, from_time: float) -> float:
        ...
        for _ in range(525600):  # Maximum number of minutes in a year
            ...
            if (minute in self.minute and
                hour in self.hour and
                day in self.day and
                month in self.month and
                day_of_week in self.day_of_week):
                return dt.timestamp()
...
class Scheduler:
        ...
        while self.running:
            try:
                ...
                if next_task_time is not None:
                    sleep_time = max(0.1, min(1.0, next_task_time - time.time()))
                else:
                    sleep_time = 1.0
    ...
    def _handle_event(self, event: Dict):
        ...
        with self.lock:
            ...
            for task in matching_tasks:
                thread = threading.Thread(
                    target=self._execute_event_task,
                    args=(task, event)
                )
                thread.daemon = True
                thread.start()
    ...
    def _execute_event_task(self, task: EventTask, event: Dict):
        ...
        """
        start_time = time.time()
        logger.debug(f"Executing event task {task.name} ({task.id}) for event {event.get('type')}")
        ...
        try:
            ...
            logger.debug(f"Event task {task.name} completed in {execution_time:.2f}s")
    ...
    def schedule_task(self, task: Task) -> str:
        ...
        with self.lock:
            if isinstance(task, ScheduledTask):
                ...
                now = time.time()
                scheduled_task = ScheduledTask(task.function, now, args=task.args, kwargs=task.kwargs, name=task.name)
    ...
    def get_stats(self) -> Dict:
        ...
        with self.lock:
            return {
                ...
                "successful_tasks": len([t for t in self.completed_tasks if t.get("status") == "success"]),
                "failed_tasks": len([t for t in self.completed_tasks if t.get("status") == "error"]),
...
def schedule_at(function: Callable, time_at: Union[float, datetime.datetime], **kwargs) -> str:
    """
    Schedule a function at a specific time.
