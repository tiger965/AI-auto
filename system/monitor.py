...
class SystemMonitor:
    ...
    def init_performance_manager(self, save_results: bool = True, results_dir: Optional[str] = None):
        """
        初始化性能测试管理器
    ...
    def _update_history_data(self, data: Dict) -> None:
        ...
        disk_percent = 0.0
        if data['disk']:
            disk_percent = sum(d['percent'] for d in data['disk'].values()) / len(data['disk'])
        ...
        try:
            ...
            status = {
                ...
                'resources': {
                    'cpu': {
                        ...
                        'status': 'normal' if data['cpu']['percent'] < self.resource_thresholds['cpu'] else 'alert'
                    },
                    'memory': {
                        'usage': data['memory']['percent'],
                        'status': 'normal' if data['memory']['percent'] < self.resource_thresholds['memory'] else 'alert'
                    },
                    'disk': {},
                }
            }
            ...
            for mount, disk_data in data['disk'].items():
                status['resources']['disk'][mount] = {
                    ...
                    'status': 'normal' if disk_data['percent'] < self.resource_thresholds['disk'] else 'alert'
                }
    ...
    def get_history_data(
        ...
        """
        # 计算需要的数据点数量
        points = min(int(hours * 60 / self.interval), len(self.history_data['timestamp']))
    ...
    def generate_report(self, report_type: str = 'daily') -> Dict:
        ...
        # 计算告警次数
        cpu_alerts = sum(1 for x in history['cpu'] if x > self.resource_thresholds['cpu'])
        memory_alerts = sum(1 for x in history['memory'] if x > self.resource_thresholds['memory'])
        disk_alerts = sum(1 for x in history['disk'] if x > self.resource_thresholds['disk'])
        ...
        if report['summary']['cpu']['avg'] > self.resource_thresholds['cpu'] * 0.8:
            report['recommendations'].append({
                'resource': 'cpu',
                'message': '考虑优化CPU密集型任务或增加CPU资源'
            })
        
        if report['summary']['memory']['avg'] > self.resource_thresholds['memory'] * 0.8:
            report['recommendations'].append({
                'resource': 'memory',
