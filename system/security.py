...
class RateLimiter:
    ...
    def _cleanup_expired_counters(self):
        ...
        now = int(time.time())
        expired_keys = [
            key for key, data in self.counters.items() if data["expires"] <= now
        ]
...
class AuthManager:
    ...
    def verify_hmac_signature(self, api_key, signature, payload, timestamp):
        ...
        expected_signature = hmac.new(
            secret, message, hashlib.sha256).hexdigest()
...
class TradingAPIGuard:
    ...
    def secure_trading_operation(
        self, operation, user_id, client_ip, payload=None, security_context=None
    ):
        """
        交易操作安全检查
        ...
        """
        # 获取操作风险级别
        risk_level = self.operation_risk_levels.get(
            operation, SecurityLevel.MEDIUM)
        ...
        allowed, remaining, reset_time = self.rate_limiter.is_allowed(
            user_id, endpoint)
        if not allowed:
            # 记录速率限制
            trading_logger.log_error(
            ...
            return (
                False,
                f"Rate limit exceeded. Try again after {datetime.fromtimestamp(reset_time)}",
            )

        # 根据风险级别执行额外检查
        check_func = self.security_level_checks[risk_level]
        is_allowed, reason = check_func(
    ...
    def _high_security_check(
        self, operation, user_id, client_ip, payload, security_context
    ):
        """
        高级别安全检查
        ...
        """
        # 首先执行中级检查
        allowed, reason = self._medium_check(
            operation, user_id, client_ip, payload, security_context
        )
        if not allowed:
    ...
    def _critical_security_check(
        ...
        """
        # 首先执行高级检查
        allowed, reason = self._high_security_check(
            operation, user_id, client_ip, payload, security_context
        )
        if not allowed:
            return allowed, reason
        ...
        try:
            last_auth = datetime.fromisoformat(
                security_context["last_auth_time"])
            now = datetime.now()
        ...
        with self.lock:
            ...
            if key not in self.suspicious_activity:
                self.suspicious_activity[key] = {
                    "operations": [], "last_check": now}

            # 添加操作记录
            self.suspicious_activity[key]["operations"].append(
                {"op": operation, "time": now}
            )
            ...
            self.suspicious_activity[key]["operations"] = [
                op
                for op in self.suspicious_activity[key]["operations"]
                if op["time"] >= cutoff
            ]
            ...
            high_risk_ops = [
                ...
                if self.operation_risk_levels.get(op["op"], SecurityLevel.LOW)
...
class APISecurityManager:
    ...
    def __init__(self, redis_client=None, jwt_secret=None):
        ...
        self.trading_guard = TradingAPIGuard(
            self.auth_manager, self.rate_limiter, self.access_control
        )
    ...
    def _configure_default_rate_limits(self):
        ...
        self.rate_limiter.configure_endpoint(
            "api/v1/market/data", 300, 60
        )  # 每分钟300次
        self.rate_limiter.configure_endpoint(
            "api/v1/user/profile", 60, 60
        )  # 每分钟60次

        # 交易相关端点
        self.rate_limiter.configure_endpoint(
            "trading/place_order", 30, 60
        )  # 每分钟30次
        self.rate_limiter.configure_endpoint(
            "trading/cancel_order", 50, 60
        )  # 每分钟50次
        self.rate_limiter.configure_endpoint(
            "trading/get_market_data", 500, 60
        )  # 每分钟500次
        self.rate_limiter.configure_endpoint(
            "trading/create_strategy", 10, 60
        )  # 每分钟10次

    def _configure_resource_permissions(self):
        """配置资源权限"""
        # 交易相关资源
        self.access_control.define_resource_permissions(
            "trading:get_market_data", {"read"}
        )
        self.access_control.define_resource_permissions(
            "trading:analyze_strategy", {"read", "analyze"}
        )
        self.access_control.define_resource_permissions(
            "trading:create_strategy", {"read", "trade"}
        )
        self.access_control.define_resource_permissions(
            "trading:modify_strategy", {"read", "trade"}
        )
        self.access_control.define_resource_permissions(
            "trading:place_order", {"read", "trade"}
        )
        self.access_control.define_resource_permissions(
            "trading:cancel_order", {"read", "trade"}
        )
        self.access_control.define_resource_permissions(
            "trading:withdraw_funds", {"read", "trade", "withdraw"}
        )
    ...
    def secure_api_request(self, endpoint, client_id, auth_data, request_data=None):
        ...
        if auth_type == "api_key":
            ...
            if not self.access_control.check_permission(
                key_info["client_id"], resource
            ):
                logger.warning(
                    f"Permission denied: {client_id} for {resource}")
            ...
            if not api_key or not signature or not timestamp:
                return False, {
                    "error": "unauthorized",
                    "message": "Missing HMAC authentication data",
                }

            # 验证HMAC签名
            payload_str = json.dumps(request_data) if request_data else ""
            if not self.auth_manager.verify_hmac_signature(
                api_key, signature, payload_str, timestamp
            ):
                logger.warning(
                    f"Invalid HMAC signature: {api_key} for {endpoint}")
            ...
            if not self.access_control.check_permission(
                ...
                logger.warning(
                    f"Permission denied: {key_info['client_id']} for {resource}"
                )
                return False, {"error": "forbidden", "message": "Permission denied"}
    ...
    def secure_trading_request(
        ...
        if not allowed:
            logger.warning(
                f"Trading operation denied: {user_id}, {operation}, reason: {reason}"
            )
            return False, {"error": "operation_denied", "message": reason}
