...
def get_prompt():
    ...
    if _prompt_config["show_status"]:
        # 使用上下文作为状态
        context = context_manager.current_context if hasattr(context_manager, 'current_context') else "ready"
    ...
    context = context_manager.current_context if hasattr(context_manager, 'current_context') else "global"
    if context == "global":
