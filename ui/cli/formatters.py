...
class ProgressMonitor:
    ...
    def __init__(self, total, description="Processing", bar_length=50):
        ...
        # 无障碍模式的声音反馈
        self.last_percent = 0
        self.sound_feedback = _settings["accessibility_mode"] and _settings["sound_feedback"]
        ...
        with self._lock:
            ...
            if _settings["accessibility_mode"]:
                ...
                if current_percent_10s > self.last_percent:
                    ...
                    # 播放进度提示音
                    if current_percent_10s > 0 and self.sound_feedback:  # 避免在开始时就播放
                        try:
                            from .sounds import play_effect
                ...
                sys.stdout.write(f"\r{self.description}: {progress_text} {suffix}")
                sys.stdout.flush()
                return
            ...
            suffix_text = safe_colorize(suffix, theme["normal"]) if suffix else ""
...
def create_spinner(description="Processing"):
    ...
    def spin():
        ...
        while not stop_spinner:
            with threading.Lock():
                sys.stdout.write(f"\r{desc} {safe_colorize(spinner_chars[i], theme['value'])}")
                sys.stdout.flush()
    ...
    for row in range(rows):
        ...
        for col in range(cols):
            ...
            if idx < len(options):
                ...
                line += item + " " * (item_width - len(item) - opt.count("\033"))
        print(line)
            ...
            if multi_select:
                ...
                try:
                    ...
                    if valid:
                        ...
                        error_msg = safe_colorize("某些选项无效，请重试。", theme["error"])
                        print(error_msg)
                        if _settings["sound_feedback"]:
                            try:
                                from .sounds import play_effect
                    ...
                    error_msg = safe_colorize("格式错误，请使用逗号分隔的数字。", theme["error"])
                    print(error_msg)
                    if _settings["sound_feedback"]:
                        try:
                            from .sounds import play_effect
                ...
                try:
                    ...
                    if 0 <= idx < len(options):
                        ...
                        error_msg = safe_colorize("无效的选择，请重试。", theme["error"])
                        print(error_msg)
                        if _settings["sound_feedback"]:
                            try:
                                from .sounds import play_effect
...
def format_table(headers, rows, border=True):
    ...
    if border:
        header_row = "│ "
        header_row += " │ ".join(safe_colorize(str(headers[i]).ljust(col_widths[i]), theme["heading"]) for i in range(len(headers)))
        header_row += " │"
        ...
        separator = "─" * sum([w + 2 for w in col_widths] + [len(col_widths) - 1])
        result.append(safe_colorize(separator, theme["border"]))
...
def format_json(data, indent=2):
    ...
    if _settings["color_enabled"]:
        ...
        formatted = re.sub(r'(true|false|null)', lambda m: safe_colorize(m.group(0), theme["value"]), formatted)
        formatted = re.sub(r'(\d+)', lambda m: safe_colorize(m.group(0), theme["number"]), formatted)
        formatted = re.sub(r'("[^"]+"):',  lambda m: safe_colorize(m.group(1), theme["key"]) + ":", formatted)
...
def format_box(text, title=None, style="single"):
    ...
    if title:
        ...
        title_line += " " + safe_colorize(title, theme["heading"]) + " " + v_char
    ...
    if _settings["accessibility_mode"]:
        ...
        if isinstance(data, dict):
            for i, (key, value) in enumerate(data.items()):
                ...
                # 添加当前节点
                lines.append("  " * indent + ("└── " if is_last_item else "├── ") + str(key))
                # 添加子节点
                if isinstance(value, (dict, list)):
                    sub_lines = format_tree(value, indent + 1, is_last_item, "")
            ...
            for i, item in enumerate(data):
                ...
                if isinstance(item, (dict, list)):
                    ...
                    lines.append("  " * indent + ("└── " if is_last_item else "├── ") + str(item))
        else:
    ...
    if isinstance(data, dict):
        ...
        for i, (key, value) in enumerate(items):
            ...
            if isinstance(value, (dict, list)) and value:
                ...
                # 递归处理子节点
                sub_tree = format_tree(value, indent + 1, is_last_item, child_prefix)
                ...
                if isinstance(value, str):
                    ...
                    value_str = safe_colorize(str(value).lower(), theme["value"])
                else:
        ...
        for i, item in enumerate(data):
            ...
            if isinstance(item, (dict, list)) and item:
                # 递归处理子列表或字典
                sub_tree = format_tree(item, indent + 1, is_last_item, prefix + ("    " if is_last else "│   "))
...
def format_output(result):
    ...
    if result_type == "exit":
        ...
        if _settings["accessibility_mode"]:
            ...
            for component, status in components.items():
                status_color = theme["success"] if status in ["ready", "active", "online", "good"] else theme["warning"]
                print(f"  {safe_colorize(component, theme['key'])}: {safe_colorize(status, status_color)}")
    ...
    # 训练相关输出
        ...
        if _settings["accessibility_mode"]:
            ...
            print(f"  {safe_colorize('状态', theme['key'])}: {safe_colorize(result.get('status', 'unknown'), theme['info'])}")
            print(f"  {safe_colorize('参数', theme['key'])}:")
            for param, value in result.get("params", {}).items():
                print(f"    {safe_colorize(param, theme['normal'])}: {safe_colorize(str(value), theme['value'])}")
        ...
        if _settings["accessibility_mode"]:
            ...
            print(f"  {safe_colorize('状态', theme['key'])}: {safe_colorize(status, status_color)}")
            
            progress = result.get("progress", {})
            print(f"  {safe_colorize('当前周期', theme['key'])}: {safe_colorize(str(progress.get('current_epoch', '?')), theme['value'])}/{safe_colorize(str(progress.get('total_epochs', '?')), theme['normal'])}")
            print(f"  {safe_colorize('完成百分比', theme['key'])}: {safe_colorize(f'{progress.get('percent', 0):.1f}%', theme['value'])}")
            print(f"  {safe_colorize('已用时间', theme['key'])}: {safe_colorize(progress.get('elapsed_time', '00:00:00'), theme['value'])}")
    ...
    # 模型信息输出
        ...
        if _settings["accessibility_mode"]:
            ...
            print(f"  {safe_colorize('名称', theme['key'])}: {safe_colorize(info.get('name', 'Unknown'), theme['value'])}")
            print(f"  {safe_colorize('版本', theme['key'])}: {safe_colorize(info.get('version', 'Unknown'), theme['value'])}")
            print(f"  {safe_colorize('参数量', theme['key'])}: {safe_colorize(info.get('parameters', 'Unknown'), theme['value'])}")
            
            print(f"\n  {safe_colorize('架构', theme['heading'])}")
            for k, v in info.get("architecture", {}).items():
                print(f"    {safe_colorize(k, theme['key'])}: {safe_colorize(str(v), theme['value'])}")
    ...
    # 可视化输出
        ...
        if viz_type == "performance":
            if _settings["accessibility_mode"]:
                ...
                for entry in data:
                    print(f"{entry.get('epoch', '?')}\t{entry.get('train_loss', 0):.4f}\t{entry.get('val_loss', 0):.4f}\t{entry.get('accuracy', 0):.4f}")
            else:
    ...
    # 评估输出
        ...
        if _settings["accessibility_mode"]:
            ...
            if "metrics" in result:
                ...
                for metric, value in result.get("metrics", {}).items():
                    print(f"    {safe_colorize(metric, theme['key'])}: {safe_colorize(str(value), theme['value'])}")
            ...
            if "details" in result:
                ...
                if "monitoring" in details:
                    ...
                    for key, value in monitoring.items():
                        print(f"    {safe_colorize(key, theme['key'])}: {safe_colorize(str(value), theme['value'])}")
                    
                    if "timestamp" in details:
                        print(f"    {safe_colorize('时间戳', theme['key'])}: {safe_colorize(details['timestamp'], theme['time'])}")
                else:
