...
def style_demo():
    ...
    demo.append(f"  {colorize('键名文本', theme['key'])}: {colorize('值文本', theme['value'])}")
    ...
    demo.append(f"  命令建议: {colorize('try help command', theme['command_suggestion'])}")
    ...
    demo.append(f"  {colorize('│', theme['border'])} 进度条: {colorize('█████░░░░░', theme['progress_bar'])} {colorize('│', theme['border'])}")
