...
class Color:
    ...
    def parse_color(color_str: str) -> ColorInfo:
        ...
        if color_str.startswith("#"):
            ...
            # HSL格式
            match = re.match(
                r"hsla?\((\d+),\s*([\d.]+)%,\s*([\d.]+)%(?:,\s*([\d.]+))?\)", color_str
            )
            if match:
            ...
            # 尝试作为颜色名称处理
            color_map = {name.lower(): code for code,
                         name in Color.COLOR_NAMES.items()}
    ...
    @staticmethod
    def mix(
        color1: Union[str, RGB], color2: Union[str, RGB], weight: float = 0.5
    ) -> str:
        """
        混合两种颜色
    ...
    @staticmethod
    def get_analogous(
        color: Union[str, RGB, HSL], angle: int = 30, count: int = 3
    ) -> List[str]:
        """
        获取类似色
    ...
    @staticmethod
    def get_split_complementary(
        color: Union[str, RGB, HSL], angle: int = 30
    ) -> List[str]:
        """
        获取分裂互补色
    ...
    def create_palette(
        ...
        if harmony == ColorHarmony.MONOCHROMATIC:
            ...
            # 补充颜色以达到指定数量
            base_hex = (
                Color.parse_color(base_color).hex
                if isinstance(base_color, str)
                else (
                    Color.rgb_to_hex(Color.hsl_to_rgb(base_color))
                    if isinstance(base_color, HSL)
                    else Color.rgb_to_hex(base_color)
                )
            )
            ...
            if count <= 3:
                ...
                for i in range(steps1):
                    ...
                    result.append(
                        Color.mix(split_colors[0], split_colors[1], weight))
                ...
                for i in range(steps2):
                    ...
                    result.append(
                        Color.mix(split_colors[1], split_colors[2], weight))
                ...
                for i in range(len(triadic_colors)):
                    ...
                    if i < len(triadic_colors) - 1:
                        ...
                        for j in range(steps):
                            ...
                            result.append(
                                Color.mix(
                                    triadic_colors[i], triadic_colors[i + 1], weight
                                )
                            )
                ...
                for i in range(len(tetradic_colors)):
                    ...
                    if i < len(tetradic_colors) - 1:
                        ...
                        for j in range(steps):
                            ...
                            result.append(
                                Color.mix(
                                    tetradic_colors[i], tetradic_colors[i + 1], weight
                                )
                            )
    ...
    def create_color_theme(
        base_color: Union[str, RGB, HSL], mood: Optional[Mood] = None
    ) -> Dict[str, str]:
        """
        创建颜色主题
        ...
        theme = {
            "primary": base_hex,
            "secondary": (
                palette[-1] if len(palette) > 1 else Color.adjust_hue(base_hex, 30)
            ),
            "background": Color.lighten(Color.desaturate(base_hex, 0.7), 0.6),
            "surface": "#FFFFFF",
            "text": Color.get_contrast_color(
                Color.lighten(Color.desaturate(base_hex, 0.7), 0.6)
            ),
            "accent": (
                palette[1] if len(
                    palette) > 2 else Color.adjust_hue(base_hex, 60)
            ),
        }
    ...
    def calculate_contrast_ratio(
        color1: Union[str, RGB], color2: Union[str, RGB]
    ) -> float:
        """
        计算两种颜色之间的对比度比率
        ...
        # 转换为RGB
        rgb1 = color1 if isinstance(
            color1, RGB) else Color.parse_color(color1).rgb
        rgb2 = color2 if isinstance(
            color2, RGB) else Color.parse_color(color2).rgb
    ...
    @staticmethod
    def is_accessible(
        color1: Union[str, RGB], color2: Union[str, RGB], level: str = "AA"
    ) -> bool:
        """
    ...
    def generate_accessible_palette(
        ...
        # 次要颜色（色相偏移60度）
        secondary_hsl = HSL(
            (adjusted_hsl.h + 60) % 360, adjusted_hsl.s, adjusted_hsl.l, adjusted_hsl.a
        )
        ...
        # 强调色（色相偏移180度）
        accent_hsl = HSL(
            (adjusted_hsl.h + 180) % 360, adjusted_hsl.s, adjusted_hsl.l, adjusted_hsl.a
        )
    ...
    def generate_material_palette(base_color: Union[str, RGB, HSL]) -> Dict[str, str]:
        ...
        for i, step in enumerate(lightness_steps):
            ...
            palette[str(level)] = Color.rgb_to_hex(
                Color.hsl_to_rgb(variant_hsl))
        ...
        # 稍微调整色相，增加饱和度
        accent_hsl = HSL(
            (base_hsl.h + 20) % 360, min(1, base_hsl.s + 0.2), base_hsl.l, base_hsl.a
        )
    ...
    def generate_random_color(
        ...
        lightness_range: Tuple[float, float] = (0.3, 0.7),
    ) -> str:
        """
        生成随机颜色
    ...
    def generate_random_palette(
        count: int = 5, harmony: ColorHarmony = ColorHarmony.MONOCHROMATIC
    ) -> List[str]:
        """
        生成随机调色板
...
def example_usage():
    ...
    print(
        f"颜色解析: {color_info.hex}, RGB: {color_info.rgb}, HSL: {color_info.hsl}")
