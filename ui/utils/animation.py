...
class Animation:
    ...
    def start(self) -> None:
        ...
        logger.debug(
            f"动画开始: {self.config.type.value}, 持续时间: {self.config.duration}秒"
        )
    ...
    def _run_animation(self) -> None:
        ...
        while not self.should_stop:
            ...
            if raw_progress >= 1.0:
                if self.config.repeat == 0 or (
                    self.config.repeat > 0
                    and self.current_iteration >= self.config.repeat - 1
                ):
                    # 动画完成
    ...
    def pause(self) -> None:
        ...
        if self.state == AnimationState.RUNNING:
            ...
            logger.debug(
                f"动画暂停: {self.config.type.value}, 进度: {self.progress:.2f}"
            )
    ...
    def resume(self) -> None:
        ...
        if self.state == AnimationState.PAUSED:
            ...
            self.state = AnimationState.RUNNING
            logger.debug(
                f"动画恢复: {self.config.type.value}, 进度: {self.progress:.2f}"
            )
...
class AnimationGroup:
    ...
    def start(self) -> None:
        ...
        logger.debug(
            f"动画组开始: 共{len(self.animations)}个动画, {'顺序' if self.sequence else '同时'}播放"
        )
    ...
    def _check_all_complete(self) -> None:
        """检查所有动画是否完成"""
        if all(
            animation.state == AnimationState.COMPLETED for animation in self.animations
        ):
            self._trigger_all_complete()
...
class FadeAnimation(Animation):
    ...
    def __init__(self, duration: float = 0.3, fade_in: bool = True, **kwargs):
        ...
        config = AnimationConfig(
            type=AnimationType.FADE, duration=duration, **kwargs)
...
class SlideAnimation(Animation):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        config = AnimationConfig(
            type=AnimationType.SLIDE, duration=duration, direction=direction, **kwargs
        )
...
class ScaleAnimation(Animation):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        config = AnimationConfig(
            type=AnimationType.SCALE, duration=duration, **kwargs)
        if config.custom_params is None:
            config.custom_params = {}
...
class RotateAnimation(Animation):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        config = AnimationConfig(
            type=AnimationType.ROTATE, duration=duration, **kwargs)
        if config.custom_params is None:
            config.custom_params = {}
...
class WaveAnimation(Animation):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        config = AnimationConfig(
            type=AnimationType.WAVE, duration=duration, **kwargs)
        if config.custom_params is None:
            config.custom_params = {}
...
class TypewriterAnimation(Animation):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        config = AnimationConfig(
            type=AnimationType.TYPEWRITER, duration=duration, **kwargs
        )
        if config.custom_params is None:
            config.custom_params = {}
...
class AnimationPresets:
    ...
    def fade_in(duration: float = 0.3, delay: float = 0.0) -> FadeAnimation:
        ...
        return FadeAnimation(
            duration=duration, fade_in=True, delay=delay, easing=EasingType.EASE_OUT
        )
    ...
    def fade_out(duration: float = 0.3, delay: float = 0.0) -> FadeAnimation:
        """淡出动画预设"""
        return FadeAnimation(
            duration=duration, fade_in=False, delay=delay, easing=EasingType.EASE_IN
        )

    @staticmethod
    def slide_in_left(
        duration: float = 0.4, distance: float = 100.0, delay: float = 0.0
    ) -> SlideAnimation:
        """从左侧滑入动画预设"""
    ...
    def gentle_pulse() -> PulseAnimation:
        ...
        return PulseAnimation(
            duration=1.2, min_scale=0.95, max_scale=1.05, easing=EasingType.EASE_IN_OUT
        )
    ...
    def typewriter(text: str, chars_per_second: float = 10.0) -> TypewriterAnimation:
        ...
        return TypewriterAnimation(
            text=text, chars_per_second=chars_per_second, easing=EasingType.LINEAR
        )
    ...
    def bouncy_entrance() -> AnimationGroup:
        """弹跳入场动画组预设"""
        fade = FadeAnimation(duration=0.3, fade_in=True,
                             easing=EasingType.EASE_OUT)

        scale = ScaleAnimation(
            duration=0.5, from_scale=0.3, to_scale=1.0, easing=EasingType.BOUNCE_OUT
        )
    ...
    def smooth_entrance() -> AnimationGroup:
        """平滑入场动画组预设"""
        fade = FadeAnimation(duration=0.4, fade_in=True,
                             easing=EasingType.EASE_OUT)
    ...
    def elegant_exit() -> AnimationGroup:
        """优雅退场动画组预设"""
        fade = FadeAnimation(duration=0.4, fade_in=False,
                             easing=EasingType.EASE_IN)

        scale = ScaleAnimation(
            duration=0.4, from_scale=1.0, to_scale=0.95, easing=EasingType.EASE_IN
        )
...
class LoadingAnimation:
    ...
    def __init__(
        self, animation_type: Type = Type.SPINNER, config: Dict[str, Any] = None
    ):
        """
        初始化加载动画
...
def stagger_animations(
    animation_factory: Callable[[], Animation], count: int, stagger_delay: float = 0.1
) -> AnimationGroup:
    """
    创建错开开始时间的多个相似动画
...
def animate_property(
    ...
    if isinstance(start_value, (int, float)) and isinstance(end_value, (int, float)):
        ...
        and start_value.startswith("#")
        and isinstance(end_value, str)
        and end_value.startswith("#")
    ):
        # 颜色类型
        config = AnimationConfig(
            type=AnimationType.FADE, duration=duration, easing=easing
        )
        ...
        def blend_color(progress):
            ...
            r1, g1, b1 = tuple(
                int(from_color[i: i + 2], 16) for i in (0, 2, 4))
        ...
        # 返回一个空动画
        config = AnimationConfig(type=AnimationType.FADE, duration=0.01)
...
def create_transitions(transitions_config: Dict[str, Dict]) -> Dict[str, Animation]:
    ...
    for key, config in transitions_config.items():
        ...
        anim_config = AnimationConfig(
            ...
            direction=(
                AnimationDirection(config.get("direction", "right"))
