...
class ThemeManager:
    ...
    DEFAULT_LIGHT_THEME = {
        ...
        "border": {"color": "#E1E2EA", "radius": "8px"},  # 边框颜色  # 边框圆角
        "shadow": {
        ...
        "font": {
            ...
            "weight": {"normal": "400", "medium": "500", "bold": "600", "heavy": "700"},
        },
    ...
    DEFAULT_DARK_THEME = {
        ...
        "border": {"color": "#38394A", "radius": "8px"},  # 边框颜色  # 边框圆角
        "shadow": {
    ...
    SEASONAL_THEMES = {
        "autumn_breeze": {
            ...
            "border": {"color": "#EDEAE2", "radius": "8px"},
            "name": "秋日微风",
    ...
    def __init__(self, config_dir: Optional[str] = None):
        ...
        if config_dir is None:
            ...
            self.config_dir = os.path.join(
                os.path.expanduser("~"), ".ai_automation")
        else:
        ...
        self.themes_file = os.path.join(
            self.config_dir, "themes", "user_themes.json")
        self.active_theme_file = os.path.join(
            self.config_dir, "themes", "active_theme.json"
        )
        ...
        try:
            if os.path.exists(self.active_theme_file):
                ...
                if self._is_valid_theme(stored_theme):
                    ...
                    logger.info(
                        f"加载活动主题: {self.active_theme.get('name', '未命名主题')}"
                    )
                else:
    ...
    def toggle_dark_mode(self) -> Dict[str, Any]:
        ...
        self._save_active_theme()
        logger.info(
            f"切换到{'亮色' if not is_dark else '深色'}模式: {new_theme.get('name')}"
        )
        return new_theme
    ...
    def create_theme(
        self, theme_data: Dict[str, Any], theme_id: Optional[str] = None
    ) -> str:
        """
        创建新的自定义主题
    ...
    def import_theme(
        self, file_path: str, new_theme_id: Optional[str] = None
    ) -> Optional[str]:
        """
        从文件导入主题
    ...
    def generate_theme_from_color(
        self, primary_color: str, is_dark: bool = False
    ) -> Dict[str, Any]:
        """
        从主色调生成完整主题
        ...
        if is_dark:
            ...
            theme_template["description"] = (
                f"以{color_name}为主调的深色主题，给人沉稳而神秘的体验"
            )
        else:
            ...
            theme_template["description"] = (
                f"以{color_name}为主调的亮色主题，给人清新愉悦的感受"
            )
    ...
    def get_css_variables(self, theme_id: Optional[str] = None) -> str:
        ...
        if "font" in theme:
            css_vars.append(
                f"  --font-family: {theme['font'].get('family', 'sans-serif')};"
            )
            ...
            if "weight" in theme["font"]:
                for weight_key, weight_value in theme["font"]["weight"].items():
                    css_vars.append(
                        f"  --font-weight-{weight_key}: {weight_value};")
...
def create_color_palette(
    base_color: str, count: int = 5, variation: str = "monochromatic"
) -> List[str]:
    """
    基于基础颜色创建调色板
