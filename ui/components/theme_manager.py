...
class ThemeManager:
    ...
    def _animate_theme_change(
        self, old_theme, new_theme, old_theme_name, new_theme_name
    ):
        """
        执行主题切换动画
    ...
    def create_theme_from_color(self, base_color, theme_id, theme_name, is_dark=False):
        ...
        r_selected, g_selected, b_selected = [
            int(x * 255) for x in selected_rgb]
    ...
    def apply_to_widget(self, widget, old_theme_name, new_theme_name, progress=1.0):
        ...
        try:
            for child in widget.winfo_children():
                self.apply_to_widget(child, old_theme_name,
                                     new_theme_name, progress)
        except:
            pass
...
if __name__ == "__main__":
    ...
    title_label = tk.Label(
        top_frame, text="主题管理器演示", font=("Helvetica", 16, "bold")
    )
    ...
    # 主题列表
    themes_label = tk.Label(sidebar, text="可用主题",
    ...
    for theme in theme_list:
        ...
        theme_button = tk.Button(
            sidebar, text=theme_name, command=make_theme_setter(theme_id), width=15
        )
