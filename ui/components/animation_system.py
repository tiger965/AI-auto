...
class Animation:
    ...
    def __init__(
        self, duration=300, easing="ease_out", on_update=None, on_complete=None
    ):
        """
        初始化动画
...
class AnimationManager:
    ...
    def scale(
        ...
        def update_scale(progress, value):
            ...
            # 更新位置和尺寸
            widget.place(
                x=current_x, y=current_y, width=current_width, height=current_height
            )
    ...
    def color_transition(
        ...
        if (
            not all(c in "0123456789ABCDEFabcdef" for c in start_color)
            or len(start_color) != 6
        ):
            start_color = "000000"
        if (
            not all(c in "0123456789ABCDEFabcdef" for c in end_color)
            or len(end_color) != 6
        ):
            end_color = "FFFFFF"
    ...
    def shake(self, widget, intensity=5, count=5, duration=None, on_complete=None):
        ...
        for i, point in enumerate(shake_points):
            ...
            animation = Animation(
                point_duration, "linear", create_update_func(
                    point[0], point[1])
            )
    ...
    def pulse(self, widget, scale_factor=1.2, count=3, duration=None, on_complete=None):
        ...
        for i in range(count):
            ...
            scale_up = Animation(
                pulse_duration,
                "ease_out",
                lambda p, v, w=widget, sf=scale_factor: self._update_pulse(
                    w, 1.0, sf, v
                ),
            )
    ...
    def highlight_attention(
        ...
        Returns:
            Animation: 动画对象
        """
        duration = duration or min(
            flash_count * 200, self.get_duration("normal"))
        ...
        for i in range(flash_count):
            ...
            to_highlight = Animation(
                flash_duration,
                "ease_out",
                lambda p, v, w=widget, c1=current_bg, c2=highlight_color: self._update_color(
                    w, c1, c2, v
                ),
            )
            ...
            to_normal = Animation(
                flash_duration,
                "ease_in",
                lambda p, v, w=widget, c1=highlight_color, c2=current_bg: self._update_color(
                    w, c1, c2, v
                ),
            )
    ...
    def _update_color(self, widget, start_color, end_color, progress):
        ...
        try:
            ...
            # 转换为RGB
            start_rgb = tuple(
                int(start_color[i: i + 2], 16) for i in (0, 2, 4))
            end_rgb = tuple(int(end_color[i: i + 2], 16) for i in (0, 2, 4))
...
if __name__ == "__main__":
    ...
    # 标题
    title = tk.Label(
        main_frame, text="动画效果演示", font=("Helvetica", 18, "bold"), bg="#f5f5f7"
    )
    ...
    def color_demo():
        animation_manager.color_transition(
            demo_widget, "#3355ff", "#ff3355", "background", duration=1000
        )
    ...
    def highlight_demo():
        animation_manager.highlight_attention(
            demo_widget, "#ffcc00", flash_count=3)
    ...
    slide_out_btn = create_animation_button(
        buttons_frame, "滑出", slide_out_demo)
    ...
    highlight_btn = create_animation_button(
        buttons_frame, "高亮", highlight_demo)
