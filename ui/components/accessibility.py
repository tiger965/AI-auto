...
class AccessibilityManager:
            ...
            if system == "Windows":
                ...
                try:
                    ...
                    comtypes.client.CreateObject(
                        "NvdaControllerClient.NvdaController")
                    nvda_running = True
                except:
                    pass
                ...
                try:
                    result = os.popen(
                        'defaults read com.apple.universalaccess voiceOverOnOffKey').read()
    ...
    def _load_settings(self):
        ...
        settings_path = os.path.join(
            self.config_dir, "accessibility_settings.json")
        ...
        if self.settings['screen_reader_support']:
            ...
            if isinstance(widget, (tk.Entry, ttk.Entry, tk.Text)):
                ...
                if isinstance(widget, (tk.Entry, ttk.Entry)):
                    ...
                    widget.bind("<<TextModified>>", lambda e,
                                w=widget: self._on_text_widget_change(w))
    ...
    def _on_text_widget_change(self, text_widget):
        ...
        if self.settings['screen_reader_support'] and self.tts_engine:
            ...
            try:
                ...
                if current_line:
                    # 只朗读当前行
                    description = text_widget._a11y_description if hasattr(
                        text_widget, '_a11y_description') else "文本区域"
                    self._speak_text(
                        f"{description}, 第{line}行: {current_line}")
            except Exception as e:
    ...
    def create_accessibility_settings_panel(self, parent):
        ...
        screen_reader_var = tk.BooleanVar(
            value=self.settings['screen_reader_support'])
        screen_reader_check = ttk.Checkbutton(
            frame,
            text="屏幕阅读器支持",
            variable=screen_reader_var,
            command=lambda: self.set_setting(
        ...
        # 键盘导航
        keyboard_nav_var = tk.BooleanVar(
        ...
        keyboard_nav_check = ttk.Checkbutton(
            ...
            command=lambda: self.set_setting(
                'keyboard_navigation', keyboard_nav_var.get())
        )
        ...
        text_zoom_label = ttk.Label(
            text_zoom_frame, text=f"{int(text_zoom_var.get() * 100)}%")
        ...
        # 减少动画
        reduced_motion_var = tk.BooleanVar(
        ...
        reduced_motion_check = ttk.Checkbutton(
            ...
            command=lambda: self.set_setting(
                'reduced_motion', reduced_motion_var.get())
        )
        ...
        cursor_size_label = ttk.Label(
            cursor_size_frame, text=f"{int(cursor_size_var.get() * 100)}%")
        ...
        hover_time_label = ttk.Label(
            hover_time_frame, text=f"{hover_time_var.get():.1f}秒")
...
if __name__ == "__main__":
    ...
    # 标题
    title_label = ttk.Label(main_frame, text="无障碍性支持演示",
    ...
    remember_check = ttk.Checkbutton(
        remember_frame, text="记住我", variable=remember_var)
    ...
    # 添加无障碍性设置面板
    settings_frame = a11y_manager.create_accessibility_settings_panel(
        main_frame)
