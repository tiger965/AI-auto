...
class SidebarNavigation:
    ...
    def _create_nav_item(self, item, index):
        ...
        fg_color = theme_config["item_fg_selected"] if is_selected else theme_config["item_fg"]
        icon_color = theme_config["icon_color_selected"] if is_selected else theme_config["icon_color"]
        ...
        if is_selected:
            ...
            # 占位框架，保持对齐
            spacer = tk.Frame(item_frame, width=4, background=theme_config["item_bg"])
        ...
        if icon:
            icon_label = tk.Label(
                ...
                background=theme_config["item_bg_selected"] if is_selected else theme_config["item_bg"]
            )
        ...
        text_label.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5, 15), pady=12)
        
        # 存储项目ID
        item_frame._item_id = item_id
        
        # 绑定事件
        item_frame.bind("<Button-1>", lambda e, id=item_id: self._on_item_click(id))
        text_label.bind("<Button-1>", lambda e, id=item_id: self._on_item_click(id))
        if icon:
            icon_label.bind("<Button-1>", lambda e, id=item_id: self._on_item_click(id))
    ...
    def _on_item_enter(self, item_frame):
        """处理鼠标进入事件"""
        # 如果已经选中，不改变样式
        if hasattr(item_frame, '_item_id') and item_frame._item_id == self.selected_item:
            return
        ...
        if item_frame.winfo_children():
            item_frame.winfo_children()[0].configure(background=theme_config["item_bg_hover"])
    ...
    def _on_item_leave(self, item_frame):
        ...
        if hasattr(item_frame, '_item_id') and item_frame._item_id == self.selected_item:
            ...
            for widget in item_frame.winfo_children():
                if isinstance(widget, tk.Label):
                    widget.configure(background=theme_config["item_bg_selected"])
            ...
            if item_frame.winfo_children():
                ...
                if isinstance(first_child, tk.Frame) and first_child.winfo_width() == 4:
                    # 这是指示器
                    first_child.configure(background=theme_config["item_fg_selected"])
            return
        ...
        # 特殊处理第一个子部件（占位符）
        if item_frame.winfo_children():
            item_frame.winfo_children()[0].configure(background=theme_config["item_bg"])
...
class TopNavigation:
    ...
    def _create_menu_item(self, item):
        ...
        if "menu" in item and isinstance(item["menu"], list):
            ...
            # 绑定事件
            menu_button.bind("<Button-1>", lambda e, m=menu: self._show_menu(e, m))
    ...
    def _animate_theme_change(self, old_theme, new_theme):
        ...
        for i in range(steps + 1):
            ...
            # 计算当前颜色 - 标题前景
            r = int(old_title_fg_rgb[0] + (new_title_fg_rgb[0] - old_title_fg_rgb[0]) * t)
            g = int(old_title_fg_rgb[1] + (new_title_fg_rgb[1] - old_title_fg_rgb[1]) * t)
            b = int(old_title_fg_rgb[2] + (new_title_fg_rgb[2] - old_title_fg_rgb[2]) * t)
            current_title_fg = f"#{r:02x}{g:02x}{b:02x}"
            
            # 计算当前颜色 - 边框
            r = int(old_border_rgb[0] + (new_border_rgb[0] - old_border_rgb[0]) * t)
            g = int(old_border_rgb[1] + (new_border_rgb[1] - old_border_rgb[1]) * t)
            b = int(old_border_rgb[2] + (new_border_rgb[2] - old_border_rgb[2]) * t)
...
class BreadcrumbNavigation:
    ...
    def _render_breadcrumbs(self):
        ...
        for i, item in enumerate(self.items):
            ...
            if i < len(self.items) - 1:
                ...
                # 最后一项使用活动颜色
                label.configure(fg=self.themes[self.current_theme]["active_fg"])
                
            # 绑定事件
            label.bind("<Button-1>", lambda e, id=item.get("id"): self._on_item_click(id))
    ...
    def _on_item_enter(self, event):
        ...
        if hasattr(event.widget, "_item_id"):
            ...
            # 应用悬停颜色
            event.widget.configure(fg=self.themes[self.current_theme]["hover_fg"])
    ...
    def _animate_add_item(self, new_item):
        ...
        # 绑定事件
        label.bind("<Button-1>", lambda e, id=new_item.get("id"): self._on_item_click(id))
...
if __name__ == "__main__":
    ...
    def on_sidebar_select(item_id):
        ...
        if item_id == "dashboard":
            breadcrumb.set_items([{"id": "home", "label": "首页"}, {"id": "dashboard", "label": "仪表盘"}])
        elif item_id == "projects":
            breadcrumb.set_items([{"id": "home", "label": "首页"}, {"id": "projects", "label": "项目管理"}])
        elif item_id == "analytics":
            breadcrumb.set_items([{"id": "home", "label": "首页"}, {"id": "analytics", "label": "数据分析"}])
        elif item_id == "automation":
            breadcrumb.set_items([{"id": "home", "label": "首页"}, {"id": "automation", "label": "自动化"}])
        elif item_id == "settings":
            breadcrumb.set_items([{"id": "home", "label": "首页"}, {"id": "settings", "label": "设置"}])
