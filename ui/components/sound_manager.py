...
class SoundManager:
    ...
    def __init__(self, sounds_dir=None, config_dir=None):
        ...
        """
        # 设置目录
        self.sounds_dir = sounds_dir or os.path.join(os.path.dirname(__file__), "../../assets/sounds")
    ...
    def play_sequence(self, sound_ids, interval=0.5, volume=None):
        ...
        threading.Thread(
            ...
            args=(sound_ids, interval, volume),
            daemon=True
        ).start()
...
if __name__ == "__main__":
    ...
    # 标题
    title_label = ttk.Label(main_frame, text="音效系统演示", font=("Helvetica", 16, "bold"))
    ...
    ui_actions = ["click", "hover", "success", "error", "notification", "alert"]
    ...
    volume_label = ttk.Label(volume_frame, text=f"音量: {int(volume_value.get() * 100)}%")
    ...
    def play_success_sequence():
        sound_manager.play_sequence(
            ["ui/click", "ui/success", "ui/complete"],
            interval=0.3
        )
    
    def play_error_sequence():
        sound_manager.play_sequence(
            ["ui/click", "ui/error", "ui/alert"],
            interval=0.3
        )
    ...
    status_label = ttk.Label(
        status_frame,
        text=f"音频后端: {sound_manager.backend or '无可用后端'}"
    )
