...
class AnalyticsView(BaseView):
    ...
    def _create_widgets(self):
        ...
        for chart_type in chart_types:
            btn = tk.Button(
                ...
                font=("Arial", 11),
                bg=theme['primary_background'] if chart_type['value'] == self.current_chart_type else theme['button_background'],
                fg=theme['primary_text'] if chart_type['value'] == self.current_chart_type else theme['button_text'],
                relief=tk.FLAT,
                padx=10,
                pady=5,
                command=lambda t=chart_type['value']: self._change_chart_type(t)
            )
        ...
        for time_range in time_ranges:
            btn = tk.Button(
                ...
                font=("Arial", 11),
                bg=theme['primary_background'] if time_range['value'] == self.current_time_range else theme['button_background'],
                fg=theme['primary_text'] if time_range['value'] == self.current_time_range else theme['button_text'],
                relief=tk.FLAT,
                padx=8,
                pady=3,
                command=lambda r=time_range['value']: self._change_time_range(r)
            )
        ...
        # 其他过滤选项
        filter_options = [
            {"label": "资产类型", "options": ["全部", "加密货币", "股票", "外汇", "商品"]},
        ...
        for filter_option in filter_options:
            ...
            dropdown.bind("<<ComboboxSelected>>", lambda e: self._update_chart())
        ...
        self.widgets['table'].configure(yscrollcommand=self.widgets['table_scrollbar'].set)
    ...
    def _layout_widgets(self):
        ...
        # 左侧过滤器和统计摘要
        self.widgets['filters_frame'].pack(side=tk.LEFT, fill='y', padx=(0, 10))
        ...
        for filter_option in ["资产类型", "交易类型"]:
            self.widgets[f"label_{filter_option}"].pack(anchor='w', pady=(0, 5))
            self.widgets[f"dropdown_{filter_option}"].pack(fill='x', pady=(0, 15))
        ...
        # 右侧图表区域
        self.widgets['chart_frame'].pack(side=tk.RIGHT, fill='both', expand=True)
        ...
        # 数据表格区域
        self.widgets['table_frame'].pack(fill='both', expand=True, padx=20, pady=10)
    ...
    def _create_summary_card(self, data, theme):
        ...
        # 添加悬停效果
        card.bind("<Enter>", lambda e, f=card: self._on_card_hover(f, True, theme))
        card.bind("<Leave>", lambda e, f=card: self._on_card_hover(f, False, theme))
    ...
    def _draw_line_chart(self, width, height, theme):
        ...
        # 过滤数据根据时间范围
        filtered_data = self._filter_data_by_time_range(self.chart_data['line'])
        ...
        for i, point in enumerate(filtered_data):
            ...
            y = height - margin['bottom'] - ((point['value'] - min_value) / (max_value - min_value)) * chart_height
            points.extend([x, y])
        ...
        for i in range(values_to_show):
            value = min_value + (i / (values_to_show - 1)) * (max_value - min_value)
            y = height - margin['bottom'] - (i / (values_to_show - 1)) * chart_height
            self.widgets['chart_canvas'].create_text(
                margin['left'] - 10, y,
                text=f"{value:.0f}",
                fill=theme['text_secondary'],
    ...
    def _draw_bar_chart(self, width, height, theme):
        ...
        for i, point in enumerate(filtered_data):
            ...
            y_top = height - margin['bottom'] - ((point['value'] - min_value) / (max_value - min_value)) * chart_height
    ...
    def _draw_pie_chart(self, width, height, theme):
        ...
        for i, item in enumerate(pie_data):
            ...
            # 创建一个颜色序列
            colors = [theme['primary_background'], theme['success_background'], 
                     theme['error_background'], theme['tag_background']]
    ...
    def _populate_table(self):
        ...
        # 示例数据
        table_data = [
            {"date": "2023-05-20", "asset": "BTC", "type": "买入", "amount": "0.01", "price": "¥282,450", "total": "¥2,824.50"},
            {"date": "2023-05-19", "asset": "ETH", "type": "卖出", "amount": "0.5", "price": "¥15,890", "total": "¥7,945.00"},
            {"date": "2023-05-18", "asset": "LTC", "type": "买入", "amount": "2.0", "price": "¥950", "total": "¥1,900.00"},
            {"date": "2023-05-15", "asset": "XRP", "type": "买入", "amount": "100", "price": "¥3.21", "total": "¥321.00"},
            {"date": "2023-05-10", "asset": "BTC", "type": "卖出", "amount": "0.005", "price": "¥281,200", "total": "¥1,406.00"},
            {"date": "2023-05-05", "asset": "ETH", "type": "买入", "amount": "0.3", "price": "¥15,750", "total": "¥4,725.00"},
            {"date": "2023-05-01", "asset": "ADA", "type": "买入", "amount": "500", "price": "¥2.85", "total": "¥1,425.00"},
            {"date": "2023-04-28", "asset": "LTC", "type": "卖出", "amount": "1.0", "price": "¥960", "total": "¥960.00"},
            {"date": "2023-04-25", "asset": "BTC", "type": "买入", "amount": "0.02", "price": "¥280,500", "total": "¥5,610.00"},
            {"date": "2023-04-20", "asset": "XRP", "type": "卖出", "amount": "50", "price": "¥3.18", "total": "¥159.00"},
        ]
