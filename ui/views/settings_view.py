...
from ui.components.sound_manager import SoundManager
...
class SettingsView(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        self.title_label = ttk.Label(
            self.title_frame, text="系统设置", font=("Arial", 16, "bold")
        )
        ...
        self.theme_title = ttk.Label(
            self.theme_frame, text="界面主题设置", font=("Arial", 12, "bold")
        )
        ...
        for setting in self.theme_settings:
            ...
            if setting["type"] == "combobox":
                widget = ttk.Combobox(
                    frame, values=setting["values"], state="readonly", width=20
                )
                widget.set(setting["default"])
                widget.bind(
                    "<<ComboboxSelected>>",
                    lambda e, name=setting["name"]: self._on_theme_change(
                        name),
                )
                ...
                color_preview = tk.Canvas(
                    color_frame, width=20, height=20, bg=setting["default"]
                )
                color_button = ttk.Button(
                    color_frame,
                    text="选择...",
                    command=lambda preview=color_preview, name=setting[
                        "name"
                    ]: self._choose_color(preview, name),
                )
                ...
                widget.bind(
                    "<Motion>",
                    lambda e, l=value_label, v=var: l.config(
                        text=str(v.get())),
                )
                ...
                scrollbar = ttk.Scrollbar(
                    text_frame, orient="vertical", command=widget.yview
                )
        ...
        self.export_theme_button = ttk.Button(
            self.theme_buttons_frame, text="导出主题", command=self._export_theme
        )

        self.import_theme_button = ttk.Button(
            self.theme_buttons_frame, text="导入主题", command=self._import_theme
        )

        self.reset_theme_button = ttk.Button(
            self.theme_buttons_frame, text="重置主题", command=self._reset_theme
        )
        ...
        self.interaction_title = ttk.Label(
            self.interaction_frame, text="交互体验设置", font=("Arial", 12, "bold")
        )
        self.interaction_title.pack(pady=10)

        # 交互效果预览区
        self.interaction_preview = ttk.LabelFrame(
            self.interaction_frame, text="效果预览"
        )
        ...
        self.sound_demo_button = ttk.Button(
            self.interaction_preview, text="音效演示", command=self._play_sound_demo
        )
        ...
        for setting in self.interaction_settings:
            ...
            if setting["type"] == "combobox":
                ...
                if setting["name"] == "sound_volume":
                    widget.bind(
                        "<ButtonRelease-1>",
                        lambda e: self.sound_manager.set_volume(
                            widget.var.get() / 100),
                    )
                ...
                if setting["name"] == "enable_sound":
                    widget.config(
                        command=lambda: self.sound_manager.set_enabled(
                            widget.var.get())
                    )
        ...
        self.accessibility_title = ttk.Label(
            self.accessibility_frame, text="无障碍选项设置", font=("Arial", 12, "bold")
        )
        ...
        for setting in self.accessibility_settings:
            ...
            if setting["type"] == "checkbox":
                ...
                if setting["name"] == "high_contrast":
                    widget.config(
                        command=lambda: self._preview_accessibility(
                            "high_contrast")
                    )
        ...
        # 快捷键设置区域
        self.shortcut_frame = ttk.LabelFrame(
            self.accessibility_frame, text="快捷键设置"
        )
        ...
        for shortcut in shortcuts:
            ...
            action_label = ttk.Label(
                shortcut_entry, text=shortcut["action"], width=15)
            key_label = ttk.Label(
                shortcut_entry, text=shortcut["key"], width=15)
        ...
        self.api_title = ttk.Label(
            self.api_frame, text="API配置设置", font=("Arial", 12, "bold")
        )
        ...
        for setting in self.api_settings:
            ...
            if setting["type"] == "entry":
                ...
                widget = ttk.Spinbox(
                    frame, from_=setting["from"], to=setting["to"], width=5
                )
                widget.set(setting["default"])
        ...
        self.api_status_indicator = ttk.Label(
            self.api_status_frame, text="未连接", foreground="red"
        )

        # 测试API连接按钮
        self.test_api_button = ttk.Button(
            self.api_frame, text="测试API连接", command=self._test_api_connection
        )
        ...
        self.notification_title = ttk.Label(
            self.notification_frame, text="通知设置", font=("Arial", 12, "bold")
        )
        ...
        for setting in self.notification_settings:
            ...
            if setting["type"] == "checkbox":
                ...
                widget = ttk.Combobox(
                    frame, values=setting["values"], state="readonly")
                widget.set(setting["default"])

                # 为通知音效添加预览功能
                if setting["name"] == "notification_sound":
                    widget.bind(
                        "<<ComboboxSelected>>", self._preview_notification_sound
                    )
        ...
        self.save_button = ttk.Button(
            self.buttons_frame, text="保存设置", command=self._save_settings
        )

        self.apply_button = ttk.Button(
            self.buttons_frame, text="应用", command=self._apply_settings
        )

        self.cancel_button = ttk.Button(
            self.buttons_frame, text="取消", command=self._cancel_changes
        )
    ...
    def _export_theme(self):
        ...
        for name, widget in self.theme_widgets.items():
            if name == "theme" or name == "font_family":
                ...
                theme_settings[name] = widget.text_widget.get(
                    1.0, tk.END).strip()
    ...
    def _import_theme(self):
        ...
        file_path = filedialog.askopenfilename(
            filetypes=[("JSON文件", "*.json"), ("所有文件", "*.*")], title="导入主题"
        )
    ...
    def _show_animation_demo(self):
        ...
        label = ttk.Label(frame, text="动画效果演示", font=("Arial", 14, "bold"))
        ...
        slide_button = ttk.Button(
            frame, text="滑动效果", command=lambda: demo_animation.slide_in(label)
        )

        pulse_button = ttk.Button(
            frame, text="脉冲效果", command=lambda: demo_animation.pulse(label)
        )
    ...
    def _preview_notification(self):
        ...
        preview_window.geometry(
            "300x100+{}+{}".format(self.winfo_rootx() +
                                   50, self.winfo_rooty() + 50)
        )
        ...
        title = ttk.Label(frame, text="交易提醒", font=("Arial", 11, "bold"))

        message = ttk.Label(
            frame, text="您的交易订单已成功执行。点击查看详情。", wraplength=250
        )

        close_button = ttk.Button(
            frame, text="关闭", width=10, command=preview_window.destroy
        )
        ...
        if animation_enabled:
            ...
            preview_window.after(
                5000, lambda: self._close_with_animation(preview_window)
            )
        else:
            # 如果没有动画，5秒后直接关闭
        ...
        if enabled:
            ...
            for frame in [
                ...
                for widget in frame.winfo_children():
                    if isinstance(widget, ttk.Label):
                        widget.config(foreground="#FFFFFF",
                                      background="#000000")
        else:
            # 恢复默认样式
            for frame in [
    ...
    def _apply_large_text(self, enabled):
        ...
        if enabled:
            ...
            for frame in [
                ...
                for widget in frame.winfo_children():
                    if isinstance(widget, ttk.Label) and not "bold" in widget.cget(
                        "font"
                    ):
    ...
    def _test_api_connection(self):
        ...
        try:
            ...
            if result:
                self.api_status_indicator.config(
                    text="连接成功", foreground="green")
