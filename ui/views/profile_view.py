...
class ProfileView(BaseView):
    ...
    def _create_widgets(self):
        ...
        self.widgets["basic_info_frame"] = tk.Frame(
            self.widgets["content_frame"], bg=theme["card_background"], padx=20, pady=20
        )

        # 用户头像
        self.widgets["avatar_frame"] = tk.Frame(
            self.widgets["basic_info_frame"], bg=theme["card_background"]
        )
        ...
        self.widgets["avatar_placeholder"].create_text(
            60, 60, text="U", fill=theme["primary_text"], font=("Arial", 48, "bold")
        )
        ...
        for field in form_fields:
            field_frame = tk.Frame(
                self.widgets["form_frame"], bg=theme["card_background"], pady=5
            )
        ...
        self.widgets["password_frame"] = tk.Frame(
            self.widgets["settings_frame"], bg=theme["card_background"], pady=10
        )
        ...
        for field in password_fields:
            field_frame = tk.Frame(
                self.widgets["password_frame"], bg=theme["card_background"], pady=5
            )
        ...
        self.widgets["separator"] = ttk.Separator(
            self.widgets["settings_frame"], orient="horizontal"
        )
        ...
        notification_options = [
            {
                "text": "接收交易确认邮件",
                "var_name": "email_confirmation",
                "default": True,
            },
            {"text": "接收安全警报", "var_name": "security_alerts", "default": True},
            {
                "text": "接收市场动态通知",
                "var_name": "market_updates",
                "default": False,
            },
        ...
        self.widgets["activity_frame"] = tk.Frame(
            self, bg=theme["card_background"], padx=20, pady=20
        )
        ...
        self.widgets["log_header"] = tk.Frame(
            self.widgets["logs_frame"], bg=theme["card_background"], pady=10
        )
        ...
        log_data = [
            {
                "time": "2023-05-20 10:23",
                "activity": "登录成功",
                "ip": "192.168.1.1",
                "status": "成功",
            },
            {
                "time": "2023-05-19 15:45",
                "activity": "修改密码",
                "ip": "192.168.1.1",
                "status": "成功",
            },
            {
                "time": "2023-05-18 08:30",
                "activity": "登录尝试",
                "ip": "203.0.113.0",
                "status": "失败",
            },
        ...
        for log in log_data:
            log_item = tk.Frame(
                self.widgets["logs_frame"], bg=theme["row_background"], padx=5, pady=8
            )

            for i, key in enumerate(["time", "activity", "ip", "status"]):
                text_color = (
                    theme["success_text"]
                    if log["status"] == "成功" and key == "status"
                    else (
                        theme["error_text"]
                        if log["status"] == "失败" and key == "status"
            ...
            # 添加悬停效果
            log_item.bind(
                "<Enter>", lambda e, f=log_item: self._on_item_hover(
                    f, True, theme)
            )
            log_item.bind(
                "<Leave>", lambda e, f=log_item: self._on_item_hover(
                    f, False, theme)
            )
    ...
    def _layout_widgets(self):
        ...
        # 左侧用户基本信息
        self.widgets["basic_info_frame"].pack(
        ...
        # 右侧安全与偏好设置
        self.widgets["settings_frame"].pack(
