...
CATEGORY_PATTERNS = [
    # 模式, 目标文件夹, 新文件名处理函数
    (r'^1[._]', 'core', lambda name: name.split('_', 1)[1] if '_' in name else name),
    (r'^2[._]', 'modules', lambda name: name.split('_', 1)[1] if '_' in name else name),
    (r'^3[._]', 'api', lambda name: name.split('_', 1)[1] if '_' in name else name),
