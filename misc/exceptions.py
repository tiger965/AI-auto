        ...
        try:
            ...
            for frame in stack_trace:
                ...
                formatted_stack.append(
                    f"{file}:{frame.lineno} in {frame.name}")
...
class ConfigNotFoundError(ConfigError):
    ...
    def __init__(
        ...
        super().__init__(message, 2001, details or {
...
class ConfigValidationError(ConfigError):
    ...
    def __init__(
        ...
        if message is None:
            message = (
                f"Configuration validation failed for keys: {', '.join(invalid_keys)}"
            )
        super().__init__(message, 2003, details or {
...
class TestExecutionError(TestError):
    ...
    def __init__(
        ...
        if message is None:
            message = (
                f"Test execution error for test '{test_id}' at step '{execution_step}'"
            )
            message += f"\nExpected: {expected}\nActual: {actual}"
        error_details = {
...
class TestCleanupError(TestError):
    ...
    def __init__(
        ...
        if message is None:
            message = (
                f"Test cleanup error for test '{test_id}' at step '{cleanup_step}'"
            )
