...
def generate_strategy_id(symbol: str, timeframe: str, prefix: str = "strat") -> str:
    ...
    try:
        ...
        # 组合ID
        strategy_id = f"{prefix}_{formatted_symbol}_{timeframe}_{timestamp}_{random_suffix}"
        
        logger.debug(f"生成策略ID: {strategy_id}, 基于符号: {symbol}, 时间周期: {timeframe}")
        return strategy_id
...
def validate_strategy_params(params: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    try:
        # 检查必填字段
        required_fields = ["symbol", "timeframe", "entry_logic", "exit_logic", "risk"]
        ...
        # 检查入场和出场逻辑
        if not isinstance(params["entry_logic"], list) or len(params["entry_logic"]) == 0:
        ...
        for logic in params["entry_logic"]:
            if logic not in supported_entry_logic and not logic.startswith("custom_"):
                ...
                return False, f"不支持的入场逻辑: {logic}, 支持的选项: {', '.join(supported_entry_logic)}"
        ...
        for logic in params["exit_logic"]:
            if logic not in supported_exit_logic and not logic.startswith("custom_"):
                ...
                return False, f"不支持的出场逻辑: {logic}, 支持的选项: {', '.join(supported_exit_logic)}"
        ...
        try:
            ...
            if stop_loss <= 0 or stop_loss > 0.5:
                logger.warning(f"策略参数验证失败: 止损比例 {stop_loss} 应在 0-50% 之间")
        ...
        if "position_size" in params:
            if not isinstance(params["position_size"], (int, float)) or params["position_size"] <= 0:
                logger.warning(f"策略参数验证失败: 仓位大小 {params['position_size']} 应为正数")
        ...
        if "indicators" in params:
            ...
            for indicator_name in params["indicators"].keys():
                if indicator_name not in supported_indicators:
                    ...
                    return False, f"不支持的指标: {indicator_name}, 支持的指标: {', '.join(supported_indicators)}"
...
def get_supported_entry_logic() -> List[str]:
    ...
    """
    entry_logic = [
        "ema_cross", "macd_cross", "rsi_oversold", 
        "bollinger_breakout", "volume_spike", "support_bounce"
    ]
...
def get_supported_exit_logic() -> List[str]:
    ...
    """
    exit_logic = [
        "ema_cross", "macd_cross", "rsi_overbought", 
...
def calculate_position_size(
    ...
    try:
        ...
        if position_size > max_possible_position:
            logger.warning(f"计算的仓位大小({position_size})超过可用余额允许的最大值({max_possible_position})，将使用最大可能值")
            position_size = max_possible_position
        
        logger.info(f"计算仓位大小: {position_size}, 基于账户余额: {account_balance}, 风险百分比: {risk_percentage}%")
...
def validate_exchange_symbol(exchange_name: str, symbol: str) -> bool:
    ...
    logger.warning(f"验证交易所交易对 {exchange_name}/{symbol} - 此功能需要集成具体交易所API")
    return True  # 仅作为示例返回
