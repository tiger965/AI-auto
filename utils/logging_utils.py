...
def setup_logging(
    ...
    # 记录启动信息
    logger = logging.getLogger(__name__)
    logger.info(f"日志系统初始化完成，应用: {app_name}, 日志级别: {logging.getLevelName(default_level)}")
    logger.info(f"系统信息: {platform.system()} {platform.release()}, Python {platform.python_version()}")
    if log_to_file:
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # 记录函数返回
                logger.log(level, f"函数 {func_name} 返回: {repr(result)}, 耗时: {execution_time:.6f}秒")
                return result
            except Exception as e:
                execution_time = time.time() - start_time
                
                # 记录函数异常
                logger.exception(f"函数 {func_name} 抛出异常: {repr(e)}, 耗时: {execution_time:.6f}秒")
                raise
...
def setup_production_logging(
    ...
    if not os.path.exists(app_log_dir):
        try:
            ...
            # 如果没有权限创建目录，使用默认位置
            app_log_dir = os.path.join(os.path.expanduser('~'), '.logs', app_name)
...
if __name__ == "__main__":
    ...
    # 测试审计日志
    audit_log("CONFIG_CHANGE", user="admin", resource="system_settings", action="update", result="success")
