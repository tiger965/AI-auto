...
class AppSettings:
    ...
    def _override_from_environment(self, settings: Dict[str, Any]) -> None:
            ...
            try:
                old_value = (
                    settings.get("app", {}).get(
                        "performance", {}).get("max_workers", 0)
                )
                workers_value = int(os.environ[max_workers_var])
                settings.setdefault("app", {}).setdefault("performance", {})[
                    "max_workers"
                ] = workers_value
                changes.append(
                    {
                ...
                logging.warning(
                    f"无效的MAX_WORKERS值: {os.environ[max_workers_var]}")
        ...
        if cache_size_var in os.environ:
            try:
                ...
                logging.warning(
                    f"无效的CACHE_SIZE值: {os.environ[cache_size_var]}")
    ...
    def validate_config(self, config: Dict[str, Any]) -> None:
        ...
        if "max_workers" in perf and (
            not isinstance(perf["max_workers"],
                           int) or perf["max_workers"] <= 0
        ):
            raise ValueError("max_workers必须是正整数")

        if "cache_size" in perf and (
            not isinstance(perf["cache_size"], int) or perf["cache_size"] < 0
        ):
