...
class APIConfig:
    ...
    def _override_from_environment(self, settings: Dict[str, Any]) -> None:
        ...
        if server_url_var in os.environ:
            ...
            settings.setdefault("server", {})["url"] = os.environ[server_url_var]
            changes.append({
                "key": "server.url",
                "old_value": old_value,
        ...
        if version_var in os.environ:
            ...
            settings.setdefault("server", {})["version"] = os.environ[version_var]
            changes.append({
                "key": "server.version",
                "old_value": old_value,
        ...
        if retries_var in os.environ:
            try:
                ...
                settings.setdefault("request", {})["max_retries"] = retries_value
                changes.append({
    ...
    def validate_config(self, config: Dict[str, Any]) -> None:
        ...
        if "timeout" in request and (not isinstance(request["timeout"], int) or request["timeout"] <= 0):
            raise ValueError("请求超时必须是正整数")
            
        # 验证最大重试次数
        if "max_retries" in request and (not isinstance(request["max_retries"], int) or request["max_retries"] < 0):
