...
class Credentials:
    ...
    def _override_from_environment(self, settings: Dict[str, Any]) -> None:
        ...
        if db_password_var in os.environ:
            settings.setdefault("database", {})["password"] = os.environ[db_password_var]
        ...
        if oauth_secret_var in os.environ:
            settings.setdefault("oauth", {})["client_secret"] = os.environ[oauth_secret_var]
    ...
    def _decrypt_sensitive_info(self, config: Dict[str, Any]) -> None:
        ...
        if "database" in config and "password_encrypted" in config["database"]:
            try:
                ...
                config["database"]["password"] = base64.b64decode(encrypted).decode('utf-8')
        ...
        if "api" in config and "key_encrypted" in config["api"]:
            try:
                ...
                config["api"]["key"] = base64.b64decode(encrypted).decode('utf-8')
        ...
        if "oauth" in config and "client_secret_encrypted" in config["oauth"]:
            try:
                ...
                config["oauth"]["client_secret"] = base64.b64decode(encrypted).decode('utf-8')
    ...
    def validate_config(self, config: Dict[str, Any]) -> None:
        ...
        if "database" in config:
            db = config["database"]
            if "user" in db and not (db.get("password") or db.get("password_encrypted")):
                raise ValueError("数据库密码不能为空")
                
        # 验证API凭证
        if "api" in config and not (config["api"].get("key") or config["api"].get("key_encrypted")):
            raise ValueError("API密钥不能为空")
            
        # 验证OAuth凭证
        if "oauth" in config:
            oauth = config["oauth"]
            if oauth.get("client_id") and not (oauth.get("client_secret") or oauth.get("client_secret_encrypted")):
    ...
    def encrypt_credentials(self, credentials: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "database" in result and "password" in result["database"]:
            try:
                ...
                result["database"]["password_encrypted"] = base64.b64encode(password.encode('utf-8')).decode('utf-8')
        ...
        if "api" in result and "key" in result["api"]:
            try:
                key = result["api"]["key"]
                result["api"]["key_encrypted"] = base64.b64encode(key.encode('utf-8')).decode('utf-8')
                del result["api"]["key"]
            except Exception as e:
        ...
        if "oauth" in result and "client_secret" in result["oauth"]:
            try:
                ...
                result["oauth"]["client_secret_encrypted"] = base64.b64encode(secret.encode('utf-8')).decode('utf-8')
    ...
    def save_credentials(self, credentials: Dict[str, Any], environment: str = "development") -> bool:
        """
        保存凭证信息
        ...
        try:
            ...
            # 保存到文件
            import json
            config_file = secure_dir / f"{environment}_{self._config_type}.json"
