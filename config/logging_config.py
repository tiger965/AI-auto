...
class LoggingConfig:
    ...
    def configure_logging(self, environment: str = "development") -> None:
        ...
        if config.get("console", {}).get("enabled", True):
            ...
            console_level = getattr(logging, config.get("console", {}).get("level", "INFO"))
            console_handler.setLevel(console_level)
            
            # 格式化器
            formatter = logging.Formatter(
                config.get("console", {}).get("format", "%(asctime)s - %(name)s - %(levelname)s - %(message)s")
            )
        ...
        if config.get("file", {}).get("enabled", False):
            ...
            # 日志文件路径
            log_file = os.path.join(log_dir, config.get("file", {}).get("filename", "app.log"))
            
            file_handler = logging.FileHandler(log_file)
            file_level = getattr(logging, config.get("file", {}).get("level", "INFO"))
            file_handler.setLevel(file_level)
            
            # 格式化器
            formatter = logging.Formatter(
                config.get("file", {}).get("format", "%(asctime)s - %(name)s - %(levelname)s - %(message)s")
            )
    ...
    def _configure_audit_logging(self, config: Dict[str, Any], environment: str) -> None:
        """配置审计日志"""
        # 获取路径配置
        from config.paths import Paths
        paths_config = Paths().get_config(environment)
        ...
        # 审计日志文件路径
        audit_file = os.path.join(log_dir, config.get("audit", {}).get("filename", "audit.log"))
    ...
    def _override_from_environment(self, settings: Dict[str, Any]) -> None:
        ...
        if console_level_var in os.environ:
            ...
            settings.setdefault("console", {})["level"] = os.environ[console_level_var]
            changes.append({
                "key": "console.level",
                "old_value": old_value,
        ...
        if file_level_var in os.environ:
            ...
            settings.setdefault("file", {})["level"] = os.environ[file_level_var]
            changes.append({
                "key": "file.level",
                "old_value": old_value,
    ...
    def validate_config(self, config: Dict[str, Any]) -> None:
        ...
        # 控制台日志级别
        if "console" in config and "level" in config["console"] and config["console"]["level"] not in valid_levels:
            raise ValueError(f"无效的控制台日志级别: {config['console']['level']}")
            
        # 文件日志级别
        if "file" in config and "level" in config["file"] and config["file"]["level"] not in valid_levels:
