...
class TradingConfig:
    ...
    def _override_from_environment(self, settings: Dict[str, Any]) -> None:
        ...
        if freqtrade_path_var in os.environ:
            ...
            settings.setdefault("freqtrade", {})["base_path"] = os.environ[
                freqtrade_path_var
            ]
            changes.append(
                {
        ...
        if strategy_path_var in os.environ:
            ...
            settings.setdefault("freqtrade", {})["strategy_path"] = os.environ[
                strategy_path_var
            ]
            changes.append(
                {
        ...
        if exchange_var in os.environ:
            ...
            settings.setdefault("exchange", {})[
                "name"] = os.environ[exchange_var]
            changes.append(
                {
                    "key": "exchange.name",
                    "old_value": old_value,
        ...
        if timeframe_var in os.environ:
            old_value = settings.get("strategy", {}).get(
                "default_timeframe", "")
            settings.setdefault("strategy", {})["default_timeframe"] = os.environ[
                timeframe_var
            ]
            changes.append(
                {
        ...
        if changes:
            ...
            for change in changes:
                audit_logger.info(
                    f"交易配置变更: {change['key']} 从 {change['old_value']} 变更为 {change['new_value']}"
                )
    ...
    def _load_exchange_credentials(
        ...
        if (
            "exchanges" in creds
            and config.get("exchange", {}).get("name") in creds["exchanges"]
        ):
