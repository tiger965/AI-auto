...
class Request(CoreModel):
    ...
    def validate_request_type(cls, v: str) -> str:
        ...
        if valid_types and v not in valid_types:
            ...
            raise ValueError(f"Invalid request type. Must be one of: {', '.join(valid_types)}")
...
class User(CoreModel):
    ...
    def validate_username(cls, v: str) -> str:
        ...
        if not v.isalnum() and not any(c in '_-.' for c in v):
            raise ValueError("Username can only contain alphanumeric characters, underscore, dash, or period")
        return v
...
class AuditLogEntry(CoreModel):
    ...
    def validate_action(cls, v: str) -> str:
        ...
        valid_actions = ['create', 'read', 'update', 'delete', 'login', 'logout', 'export', 'import', 'execute']
...
class ModelRegistry:
    ...
    def register(self, name: str, model_class: Type[CoreModel]) -> None:
        ...
        if name in self._models:
            self._logger.warning(f"Overriding existing model registration for {name}")
