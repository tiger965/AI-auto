...
class WorkflowStep:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self._logger.warning(f"Failed to load config for section {self._config_section}: {e}")
            return {}
    ...
    def execute(self, context: Context) -> StepResult:
        ...
        try:
            ...
            context.set(f'step_result.{self._id}', {
    ...
    def _execute(self, context: Context) -> StepResult:
        ...
        raise NotImplementedError("WorkflowStep subclasses must implement _execute()")
...
class Workflow:
    ...
    def remove_step(self, step_id: str) -> bool:
        ...
        if step_id in self._steps:
            ...
            self._logger.debug(f"Removed step {step_id} from workflow {self._id}")
            return True
    ...
    def execute(self, context: Context) -> Dict[str, Any]:
        ...
        self._logger.info(f"Executing workflow {self._id} (execution_id={workflow_execution_id})")
        ...
        for step_id in self._step_order:
            if step_id not in self._steps:
                self._logger.warning(f"Step {step_id} not found in workflow {self._id}")
                continue
            ...
            if not step.validate(context):
                self._logger.warning(f"Step {step_id} validation failed, skipping")
                results[step_id] = {'status': 'skipped', 'reason': 'validation_failed'}
                continue
            ...
            if step_result.get('status') == 'error' and self._config.get('abort_on_error', True):
                self._logger.warning(f"Aborting workflow {self._id} due to step {step_id} error")
                success = False
                break
        ...
        self._logger.info(f"Workflow {self._id} completed in {end_time - start_time:.3f}s (success={success})")
