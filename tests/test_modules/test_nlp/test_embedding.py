...
from modules.nlp.embedding import TextEmbedding
...
class TestEmbedding(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_data_path = os.path.join(
            os.path.dirname(__file__), "test_data", "embedding_test_data.json"
        )
        os.makedirs(os.path.dirname(test_data_path), exist_ok=True)

        # 将不可JSON序列化的数据转为可序列化格式
        serializable_data = {
            k: v for k, v in self.test_texts.items() if k != "similar_pairs"
        }
        serializable_data["similar_pairs"] = [
            list(pair) for pair in self.test_texts["similar_pairs"]
        ]
        ...
        for key in ["simple", "sentence"]:
            self.cached_embeddings[key] = self.embedding_model.get_embedding(
                self.test_texts[key]
            )

        cached_emb_path = os.path.join(
            os.path.dirname(__file__), "test_data", "cached_embeddings.pkl"
        )
    ...
    def test_embedding_output_shape(self):
        ...
        embeddings = self.embedding_model.get_embeddings(
            self.test_texts["multi_lang"])
        self.assertIsInstance(embeddings, np.ndarray)
        self.assertEqual(
            embeddings.shape,
            (len(self.test_texts["multi_lang"]),
    ...
    def test_embedding_consistency(self):
        ...
        for key in self.cached_embeddings:
            current_emb = self.embedding_model.get_embedding(
                self.test_texts[key])
            np.testing.assert_allclose(
                current_emb, self.cached_embeddings[key], rtol=1e-5
            )
    ...
    def test_embedding_similarity(self):
        ...
        for text1, text2 in self.test_texts["similar_pairs"]:
            ...
            # 相似文本的嵌入应该有较高相似度
            self.assertGreater(
                similarity, 0.5, f"'{text1}'和'{text2}'的嵌入相似度太低: {similarity}"
            )
    ...
    def test_embedding_normalization(self):
        """测试嵌入向量归一化"""
        emb = self.embedding_model.get_embedding(
            self.test_texts["simple"], normalize=True
        )
        # 归一化后的向量范数应约等于1
    ...
    def test_embedding_benchmark(self):
        ...
        # 记录性能指标
        print(
            f"嵌入性能基准测试: 短文本: {simple_time:.4f}秒, 长段落: {paragraph_time:.4f}秒, 批量: {batch_time:.4f}秒"
        )
