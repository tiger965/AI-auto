...
from modules.nlp.sentiment import SentimentAnalyzer
...
class TestSentimentAnalysis(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_data_path = os.path.join(
            os.path.dirname(__file__), "test_data", "sentiment_test_data.json"
        )
    ...
    def test_sentiment_classification_chinese(self):
        ...
        for sentiment, texts in self.test_data["chinese"].items():
            for text in texts:
                ...
                self.assertEqual(
                    result["sentiment"], sentiment, f"中文文本'{text}'情感分析错误"
                )
                self.assertIn("confidence", result)
                self.assertGreater(
                    result["confidence"], 0.6, f"中文文本'{text}'情感分析置信度过低"
                )
    ...
    def test_sentiment_classification_english(self):
        ...
        for sentiment, texts in self.test_data["english"].items():
            for text in texts:
                ...
                self.assertIn("sentiment", result)
                self.assertEqual(
                    result["sentiment"], sentiment, f"英文文本'{text}'情感分析错误"
                )
                self.assertIn("confidence", result)
                self.assertGreater(
                    result["confidence"], 0.6, f"英文文本'{text}'情感分析置信度过低"
                )
    ...
    def test_sentiment_classification_complex(self):
        ...
        for text in self.test_data["complex"]:
            ...
            # 这些复杂的例子应该被识别为正面情感
            self.assertEqual(
                result["sentiment"], "positive", f"复杂情感文本'{text}'分析错误"
            )
    ...
    def test_sentiment_intensity(self):
        ...
        weak_result = self.sentiment_analyzer.analyze(
            weak_positive, include_intensity=True
        )
        strong_result = self.sentiment_analyzer.analyze(
            strong_positive, include_intensity=True
        )

        self.assertIn("intensity", weak_result)
        self.assertIn("intensity", strong_result)
        self.assertGreater(
            strong_result["intensity"],
            weak_result["intensity"],
            "强烈情感文本的强度应高于弱情感文本",
        )
    ...
    def test_sentiment_aspects(self):
        ...
        # 验证识别出的方面
        aspects = {aspect["name"]: aspect["sentiment"]
    ...
    def test_sentiment_benchmark(self):
        ...
        for _ in range(20):
            self.sentiment_analyzer.analyze(
                self.test_data["chinese"]["positive"][0])
        ...
        # 记录性能指标
        print(
            f"情感分析性能基准测试: 单个文本: {single_time:.4f}秒, 批量({len(all_texts)}个文本): {batch_time:.4f}秒"
        )
    ...
    def test_sentiment_model_consistency(self):
        ...
        for i in range(1, len(results)):
            ...
            self.assertAlmostEqual(
                results[0]["confidence"], results[i]["confidence"], places=5
            )
