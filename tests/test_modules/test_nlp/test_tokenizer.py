...
from modules.nlp.tokenizer import Tokenizer
...
class TestTokenizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_data_path = os.path.join(
            os.path.dirname(__file__), "test_data", "tokenizer_test_data.json"
        )
    ...
    def test_tokenize_with_custom_config(self):
        ...
        tokens = self.tokenizer.tokenize(
            self.test_texts["english"], config=config)
        self.assertIsInstance(tokens, list)
        self.assertGreater(len(tokens), 0)
        # 检查是否小写化
        for token in tokens:
    ...
    def test_tokenize_benchmark(self):
        ...
        # 记录性能指标
        print(
            f"分词性能基准测试: 中文: {chinese_time:.4f}秒, 英文: {english_time:.4f}秒"
        )
