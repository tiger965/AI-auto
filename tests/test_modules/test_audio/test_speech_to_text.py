...
# 假设语音转文本模块的导入路径
try:
from modules.audio import speech_to_text
except ImportError:
    # 提供一个模拟的语音转文本函数用于测试
...
class TestSpeechToText(unittest.TestCase):
    ...
    def _create_test_audio_files(self):
        ...
        for lang in self.supported_languages:
            ...
            with wave.open(file_path, 'wb') as wf:
                ...
                wf.writeframes(
                    np.zeros(16000, dtype=np.int16).tobytes())  # 1秒静音
    ...
    def test_low_quality_audio(self):
        """测试低质量音频的语音识别性能"""
        # 创建低质量（低采样率）的测试音频文件
        low_quality_audio_path = os.path.join(
            self.temp_dir, "low_quality_test.wav")
    ...
    def test_real_microphone_input(self):
        ...
        try:
            ...
            subprocess.run(["arecord", "-l"], stdout=subprocess.PIPE,
        ...
        try:
            # 录制3秒音频
            subprocess.run(
                ["arecord", "-d", "3", "-f", "S16_LE", "-r",
                    "16000", "-c", "1", mic_audio_path],
