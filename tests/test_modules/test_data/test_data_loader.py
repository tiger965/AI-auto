...
class TestDataLoader(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        test_data = {"items": [{"id": i, "name": f"item_{i}", "value": i * 10} for i in range(100)]}
        ...
        with open(cls.large_csv_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["id", "data1", "data2", "data3", "data4", "data5"])
            for i in range(1000000):  # 约50MB数据
                writer.writerow([i, i*2, i*3, f"text_{i}", f"category_{i%10}", i/100])
    ...
    def test_load_performance(self):
        ...
        self.assertEqual(list(df.columns), ["id", "data1", "data2", "data3", "data4", "data5"])
