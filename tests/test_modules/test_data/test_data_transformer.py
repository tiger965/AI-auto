    ...
    try:
        ...
        class DataTransformer:
            ...
            def one_hot_encode(self, data, column):
                ...
                # 将编码结果与原数据合并
                result = pd.concat([data.drop(column, axis=1), encoded], axis=1)
                return result
...
class TestDataTransformer(unittest.TestCase):
    ...
    def test_one_hot_encode(self):
        ...
        # 验证结果正确性
        self.assertEqual(len(encoded_df.columns), len(self.mixed_df.columns) - 1 + 4)  # 原列数 - 1 + 分类数量
    ...
    def test_batch_process(self):
        ...
        # 测试批量处理DataFrame
        batch_size = 100
        processed_df = self.transformer.batch_process(self.numeric_df, batch_size, double_values)
        ...
        # 测试批量处理NumPy数组
        processed_array = self.transformer.batch_process(self.numpy_array, batch_size, lambda x: x * 2)
    ...
    def test_performance(self):
        ...
        normalized_large_df = self.transformer.normalize(self.large_df[['value1', 'value2']])
        end_time = time.time()
        ...
        # 测试大规模数据的独热编码性能
        start_time = time.time()
        encoded_large_df = self.transformer.one_hot_encode(self.large_df, 'category')
        end_time = time.time()
        ...
        self.assertEqual(encoded_large_df.shape[1], self.large_df.shape[1] - 1 + 4)  # 原列数 - 1 + 分类数量
