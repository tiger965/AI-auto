...
class TestImageProcessor(unittest.TestCase):
    ...
    def test_apply_filter(self):
        ...
        # 测试高斯模糊
        blurred = self.image_processor.apply_gaussian_blur(img, kernel_size=(5, 5))
        self.assertEqual(blurred.shape, img.shape)
        
        # 测试中值滤波
        median_filtered = self.image_processor.apply_median_filter(img, kernel_size=5)
    ...
    def test_edge_detection(self):
        ...
        # 测试Canny边缘检测
        edges = self.image_processor.detect_edges(gray, low_threshold=50, high_threshold=150)
    ...
    def test_performance(self):
        ...
        # 输出性能信息
        print(f"Performance - Load: {load_time:.4f}s, Resize: {resize_time:.4f}s")
