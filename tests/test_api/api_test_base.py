...
class APITestBase:
    ...
    def register_mock_response(self, endpoint: str, method: str, 
                              response_data: Any, status_code: int = 200):
        """注册模拟响应"""
    ...
    def _make_request(self, method: str, endpoint: str, 
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                self.logger.error(f"请求异常 (尝试 {attempt+1}/{self.max_retries}): {str(e)}")
        ...
        try:
            ...
            for key, type_info in expected_schema.items():
                ...
                if not isinstance(response_data[key], type_info):
                    self.logger.error(f"字段类型不匹配: {key}, 期望 {type_info}, 实际 {type(response_data[key])}")
                    return False
