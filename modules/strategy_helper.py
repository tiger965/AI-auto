...
class SignalGenerator:
    ...
    def trailing_stop(dataframe, atr_period=14, multiplier=2.0, direction="long"):
        ...
        if direction.lower() == "long":
            ...
            return SignalGenerator.trailing_stop(
                dataframe, atr_period, multiplier, "long"
            )
...
class StrategyTemplateGenerator:
    ...
    def generate_strategy_template(
        ...
        formatted_name = "".join(word.capitalize()
                                 for word in strategy_name.split())
...
def generate_complete_strategy(
    strategy_name, indicator_settings, entry_settings, exit_settings
):
    """
    生成完整的策略代码
    ...
    for indicator in indicator_settings:
        ...
        if hasattr(IndicatorLibrary, indicator_type):
            ...
            indicators.append(
                indicator_method("dataframe", **indicator.get("params", {}))
            )
    ...
    if entry_settings:
        for entry in entry_settings:
            ...
            entry_conditions.append(
                SignalGenerator.entry_condition(
                    entry.get("name", "default"), conditions, operator
                )
            )
    ...
    if exit_settings:
        for exit in exit_settings:
            ...
            exit_conditions.append(
                SignalGenerator.exit_condition(
                    exit.get("name", "default"), conditions, operator
                )
            )

    # 生成策略模板
    return StrategyTemplateGenerator.generate_strategy_template(
        strategy_name, indicators, entry_conditions, exit_conditions
    )