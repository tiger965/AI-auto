...
class LoaderConfig:
    ...
    def __post_init__(self):
        """数据校验和默认值设置"""
        valid_formats = ["auto", "csv", "json", "xml", "yaml", "excel", "text", "binary", "parquet", "feather", "pickle", "hdf5", "sql"]
    ...
    def _load_hdf5(self, file_obj: BinaryIO, config: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            if hasattr(file_obj, 'name') and isinstance(file_obj.name, str):
                ...
                # 对于文件对象，需要先保存到临时文件
                self._ensure_temp_dir()
                temp_path = os.path.join(self.temp_dir, f"temp_hdf5_{id(file_obj)}.h5")
        ...
        try:
            ...
            if isinstance(data, pd.DataFrame):
                ...
                if format == 'csv':
                    ...
                    data.to_hdf(output_path, key=kwargs.get('key', 'data'), **kwargs)
                else:
                    # 默认导出为CSV
                    data.to_csv(output_path, **kwargs)
