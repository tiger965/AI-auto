...
class TextToSpeech:
    ...
    def __init__(self, config: Optional[Union[Dict[str, Any], SpeechSynthesisConfig]] = None):
        ...
        logger.info(f"语音合成器初始化，引擎: {self.config.engine}, 语言: {self.config.language}")
        ...
        if self.config.engine == "gtts":
            ...
            if not PYTTSX3_AVAILABLE:
                ...
                try:
                    ...
                    # 设置属性
                    self.engine.setProperty('rate', int(self.engine.getProperty('rate') * self.config.rate))
                    ...
                    if voices:
                        ...
                        if self.config.gender == "male":
                            male_voices = [v for v in voices if "male" in v.gender.lower()]
                            if male_voices:
                                self.engine.setProperty('voice', male_voices[0].id)
                        elif self.config.gender == "female":
                            female_voices = [v for v in voices if "female" in v.gender.lower()]
                            if female_voices:
        ...
        try:
            ...
            if output_file:
                ...
                if self.config.audio_format != "wav":
                    self._convert_audio_format_file(temp_path, output_file, "wav", self.config.audio_format)
                else:
                    # 直接移动文件
                    import shutil
    ...
    def _synthesize_azure(self, 
        ...
        try:
            # 创建语音合成器
            speech_synthesizer = speechsdk.SpeechSynthesizer(speech_config=self.engine)
    ...
    def _convert_audio_format(self, 
        ...
        try:
            ...
            # 转换格式
            result = self._convert_audio_format_file(input_file, output_file, input_format, output_format)
