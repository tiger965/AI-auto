...
class SpeechRecognitionConfig:
    ...
    def __post_init__(self):
        """数据校验和默认值设置"""
        supported_engines = ["system", "google",
                             "sphinx", "wit", "azure", "whisper"]
...
class SpeechToText:
    ...
    def __init__(self, config: Optional[Union[Dict[str, Any], SpeechRecognitionConfig]] = None):
        ...
        logger.info(
            f"语音识别器初始化，引擎: {self.config.engine}, 语言: {self.config.language}")
            ...
            with microphone as source:
                ...
                if duration is not None:
                    ...
                    audio_data = self.recognizer.listen(
                        source, timeout=self.config.timeout)
                    logger.info("录音完成")
...
def transcribe_long_audio(audio_file: str,
    ...
    try:
        ...
        for i, (start, end) in enumerate(segments):
            logger.info(
                f"处理音频段 {i+1}/{len(segments)}: {start:.1f}s - {end:.1f}s")
