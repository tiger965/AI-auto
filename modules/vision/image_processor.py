...
class ImageProcessor:
    ...
    def __init__(self, config: Optional[Union[Dict[str, Any], ImageConfig]] = None):
        ...
        logger.info(
            f"图像处理器初始化，格式: {self.config.format}, 最大尺寸: {self.config.max_size}"
        )
    ...
    def preprocess_image(self, image: np.ndarray) -> np.ndarray:
        ...
        # 调整大小
        if (
            self.config.resize_width is not None
            and self.config.resize_height is not None
        ) or self.config.max_size:
            image = resize_image(
                image,
    ...
    def save_image(
        self, image: np.ndarray, path: Union[str, Path], format: Optional[str] = None
    ) -> bool:
        """
        保存图像到文件
        ...
        try:
            ...
            if PIL_AVAILABLE:
                ...
                if filter_type == "blur":
                    ...
                    filtered = ImageEnhance.Sharpness(
                        pil_image).enhance(factor)
                    ...
                    filtered = ImageEnhance.Brightness(
                        pil_image).enhance(factor)
                ...
                if filter_type == "blur":
                    ...
                    hsv[..., 2] = np.clip(
                        hsv[..., 2] * factor, 0, 255).astype(np.uint8)
                    return cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)

                elif filter_type == "contrast":
                    factor = kwargs.get("factor", 1.5)
                    return np.clip(
                        (image.astype(np.float32) - 128) * factor + 128, 0, 255
                    ).astype(np.uint8)

                else:
                    logger.warning(
                        f"未知的或不支持的OpenCV滤镜类型: {filter_type}，返回原始图像"
                    )
                    return image
    ...
    def rotate_image(
        self, image: np.ndarray, angle: float, keep_size: bool = True
    ) -> np.ndarray:
        """
        旋转图像
        ...
        try:
            if PIL_AVAILABLE:
                ...
                rotated = pil_image.rotate(
                    angle, expand=expand, fillcolor=fill_color)
                ...
                if keep_size:
                    ...
                    new_width = int(
                        np.abs(width * np.cos(angle_rad))
                        + np.abs(height * np.sin(angle_rad))
                    )
                    new_height = int(
                        np.abs(height * np.cos(angle_rad))
                        + np.abs(width * np.sin(angle_rad))
                    )
