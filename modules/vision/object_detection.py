...
class ObjectDetector:
    ...
    def __init__(self,
        ...
        if model_path is None:
            self.model_path = DETECTION_CONFIG.get(
                "model_path", "models/detection/default")
        else:
        ...
        logger.info(
            f"对象检测器初始化，类型: {self.model_type}, 置信度阈值: {self.confidence_threshold}")
    ...
    def _initialize_opencv_model(self):
        ...
        try:
            ...
            for ext in config_extensions:
                path = os.path.join(
                    model_dir, os.path.splitext(model_name)[0] + ext)
                if os.path.exists(path):
                    config_path = path
                    break
    ...
    def _initialize_yolo_model(self):
        ...
        try:
            ...
            # 查找配置文件
            cfg_path = os.path.join(model_dir, os.path.splitext(
                os.path.basename(weights_path))[0] + ".cfg")
            if not os.path.exists(cfg_path):
                # 尝试在同目录下查找任何.cfg文件
                cfg_files = [f for f in os.listdir(
                    model_dir) if f.endswith(".cfg")]
                if cfg_files:
    ...
    def _initialize_torch_model(self):
        ...
        try:
            ...
            # 确定设备
            device = torch.device(
                "cuda" if self.enable_gpu and torch.cuda.is_available() else "cpu")
            ...
            for i, pred in enumerate(predictions):
                ...
                for box, score, label in zip(boxes, scores, labels):
                    if score >= self.confidence_threshold:
                        ...
                        # 获取类别名称
                        class_id = int(label)
                        class_name = self.class_names[class_id - 1] if class_id - 1 < len(
                            self.class_names) else f"class_{class_id}"
