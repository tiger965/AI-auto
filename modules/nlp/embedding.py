...
class EmbeddingModel:
    ...
    def __init__(self, 
        ...
        if self.model_type not in self.SUPPORTED_TYPES:
            logger.warning(f"不支持的嵌入模型类型: {self.model_type}，使用默认类型: word2vec")
        ...
        try:
            if self.model_path and os.path.exists(self.model_path):
                if self.model_type == "word2vec" and GENSIM_AVAILABLE:
                    self.model = KeyedVectors.load_word2vec_format(self.model_path, binary=self.kwargs.get('binary', True))
    ...
    def get_word_vector(self, word: str) -> np.ndarray:
        ...
        try:
            if self.model_type in ["word2vec", "fasttext", "glove"] and GENSIM_AVAILABLE and self.model is not None:
    ...
    def get_text_embedding(self, text: str, tokens: Optional[List[str]] = None) -> np.ndarray:
        """
        获取文本的嵌入向量
        ...
        返回:
            文本嵌入向量
        """
        if self.model_type == "bert" and TRANSFORMERS_AVAILABLE and self.model is not None:
...
def load_embedding_model(model_config: Dict[str, Any]) -> EmbeddingModel:
    ...
    return EmbeddingModel(
        ...
        device=device,
        **{k: v for k, v in model_config.items() if k not in ['model_type', 'model_path', 'embedding_dim', 'device']}
    )