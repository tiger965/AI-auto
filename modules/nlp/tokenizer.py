...
class Tokenizer:
    ...
    def __init__(self, config: Optional[Union[Dict[str, Any], TokenizerConfig]] = None):
        ...
        logger.info(f"初始化分词器: {self.config.tokenizer_type}, 语言: {self.config.language}")
    ...
    def _initialize_tokenizer(self):
        ...
        if self.config.tokenizer_type == "jieba" and JIEBA_AVAILABLE:
            ...
            try:
                ...
                logger.error(f"无法加载SpaCy模型: {self.config.spacy_model}，回退到基础分词器")
    ...
    def _remove_stopwords(self, tokens: List[str]) -> List[str]:
        ...
        if self.config.tokenizer_type == "spacy" and SPACY_AVAILABLE:
            # 使用SpaCy的停用词
            stopwords.update([token.text for token in self.nlp('').vocab if token.is_stop])
